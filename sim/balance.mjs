...
const P = {
  ...
  investigatorReliability: 0.80, // fiabilité d'une enquête (assistant/policier…)
  ...
  majordomeGuessRate: 0.30,      // proba que le majordome garde précisément la cible du Tueur (→ échange mortel)
...
function rint(n) { return Math.floor(srand() * n); }
...
function weightedPick(items, wfn) {
  ...
  let r = srand() * total;
  for (const x of items) { r -= wfn(x); if (r <= 0) return x; }
...
const ROLES = [
  ...
  { slug: "assistant_du_detective", faction: "Civil", type: "INVESTIGATION", dw: 1.0, min: 6, invest: true },
  ...
  { slug: "medecin_legiste", faction: "Civil", type: "INVESTIGATION", dw: 1.0, min: 6, investDead: true },
...
function bracket(n) { return n <= 8 ? "small" : n <= 13 ? "mid" : "large"; }
...
const CIVIL_QUOTAS = {
  small: { INVESTIGATION: [1, 2], PROTECTEUR: [0, 1], TUEUR: [0, 1], SUPPORT: [1, 2], BOULET: [0, 0] },
  mid: { INVESTIGATION: [2, 2], PROTECTEUR: [1, 1], TUEUR: [1, 1], SUPPORT: [1, 2], BOULET: [0, 1] },
  large: { INVESTIGATION: [2, 3], PROTECTEUR: [1, 2], TUEUR: [1, 1], SUPPORT: [2, 2], BOULET: [0, 1] },
};
function acolytesCountFor(n) { return n <= 9 ? 1 : 2; } // RÈGLE LIVRÉE : plafond 2 acolytes (3 méchants max)
function neutresCountFor(n) { return n <= 7 ? 0 : n <= 11 ? 1 : 2; }
...
function drawByQuotas(pool, quotas, count) {
  ...
  for (const [t, [mn]] of Object.entries(quotas)) {
    for (let k = 0; k < mn && picked.length < count; k++) {
      ...
      const r = weightedPick(cand, (x) => x.dw); used.add(r.slug); picked.push(r);
    }
  }
  // 2) remplir le reste en respectant max
  const typeCount = () => { const c = {}; for (const r of picked) c[r.type] = (c[r.type] || 0) + 1; return c; };
  let guard = 0;
  while (picked.length < count && guard++ < 200) {
    const c = typeCount();
    const cand = pool.filter((r) => !used.has(r.slug) && (c[r.type] || 0) < (quotas[r.type]?.[1] ?? 0));
    if (cand.length === 0) break;
    const r = weightedPick(cand, (x) => x.dw); used.add(r.slug); picked.push(r);
  }
  return picked;
...
function drawRoles(n) {
  ...
  const slugs = [];
  const take = (r) => { if (r && !slugs.includes(r.slug)) slugs.push(r.slug); };
  ...
  const acoPool = eligible.filter((r) => r.faction === "Méchant" && r.type !== "TUEUR" && !slugs.includes(r.slug));
  ...
  let neuPool = eligible.filter((r) => r.faction === "Neutre" && r.slug !== "chasseur_de_vampire" && !slugs.includes(r.slug));
  ...
  if (slugs.length < n) {
    ...
    for (const r of shuffle(fb)) { if (slugs.length >= n) break; take(r); }
  }
...
function makeGame(n) {
  ...
  const players = slugs.map((slug, i) => {
    ...
    return {
      id: i, slug, originSlug: slug, faction: r.faction, type: r.type, role: r,
      alive: true, free: true,
      isTueur: slug === "tueur" || r.type === "TUEUR" && r.faction === "Méchant",
      converted: false, poisoned: false, linkedWith: null,
      shieldUntil: -1, blockedUntil: -1,
      vigilanteUsed: false, gamblerUsed: false, fioleUsed: false, transformed: false,
      ...
      prediction: slug === "oracle" ? (srand() < 0.45 ? "Méchant" : srand() < 0.82 ? "Civil" : "Neutre") : null,
    };
  });
  ...
  if (ent) {
    ...
    const a = pick(others); let b = pick(others.filter((p) => p.id !== a.id));
    if (b) { a.linkedWith = b.id; b.linkedWith = a.id; ent.pairIds = [a.id, b.id]; }
  }
  return players;
}
...
function evaluateWin(players) {
  ...
  // — Citoyens
  const entFactionAlive = entActive ? alive.filter((p) => p.slug === "entremetteur" || p.linkedWith != null).length : 0;
  const blocking = alive.filter((p) => {
    if (!isBlockingNeutre(p, players)) return false;
    if (p.slug === "entremetteur" && !entActive) return false;
    return true;
  }).length;
  if (mechAlive === 0 && vampAlive === 0 && entFactionAlive === 0 && blocking === 0) return { winner: "Civil" };
...
function runInvestigations(players, cycle) {
  const investigators = players.filter((p) => p.alive && p.free && p.faction === "Civil" && (p.role.invest || p.role.investPrison));
  ...
  for (const inv of investigators) {
    ...
    let reliability = (typeof inv.role.invest === "number" ? inv.role.invest : 1) * P.investigatorReliability;
    ...
    if (inv.slug !== "policier" && isMechantRole(t) && (t.role.cover || t.isTueur || players.some((q) => q.alive && q.free && q.role.cover && isMechantRole(q)))) {
    ...
    if (chance(reliability)) {
      // résultat correct
      if (trueMech) { t.suspByTown += 0.6; if (chance(0.5)) t.revealedMechant = true; }
      else { t.suspByTown -= 0.3; if (chance(0.5)) t.cleared = true; } // innocenté confirmé
    } else {
      // résultat trompeur
      if (trueMech) { t.suspByTown -= 0.2; }
      else { t.suspByTown += 0.3; }
    }
  }
...
function runVote(players) {
  ...
  const townTop = [...(cands.length ? cands : voters)].sort((a, b) => b.suspByTown - a.suspByTown)[0];
  ...
  for (const v of voters) {
    ...
    if (v.faction === "Neutre") { if (chance(0.5) && townTop) addVote(townTop.id); continue; }
    ...
    if (chance(P.townCoordination) && townTop && townTop.id !== v.id) addVote(townTop.id);
      ...
      const my = [...voters].filter((p) => p.id !== v.id).sort((a, b) => b.suspByTown - a.suspByTown)[0];
      if (my) addVote(my.id);
    }
  }
  if (tally.size === 0) return null;
  let best = null, bestN = -1;
  for (const [id, nv] of tally) if (nv > bestN) { bestN = nv; best = id; }
...
function killPlayer(p, players, byMechant = false) {
  ...
  p.alive = false; p.free = false;
  // chaîne amoureux
  if (p.linkedWith != null) {
    const lover = players.find((q) => q.id === p.linkedWith);
    if (lover && lover.alive) { lover.alive = false; lover.free = false; }
  }
  return true;
}
...
function mechantKill(players) {
  ...
  // protections
  const guardians = players.filter((p) => p.alive && p.free && p.faction === "Civil" && p.role.guard);
  ...
  let shielded = false;
  for (const s of shielders) { if (chance(P.majordomeGuessRate)) { shielded = true; break; } }
  ...
  for (const g of guardians) {
    if (chance(P.majordomeGuessRate)) {
      ...
      killPlayer(g, players); killPlayer(tueur, players); succession(players);
      return;
    }
  }
  if (shielded) return;
...
function neutralActions(players, cycle) {
  ...
  if (imi && cycle >= 2) {
    ...
    if (deads.length) {
      ...
      imi.faction = model.faction; imi.slug = model.slug; imi.type = model.type; imi.role = model.role;
  ...
  const gam = players.find((p) => p.alive && p.free && p.slug === "parieur_tricheur" && !p.gamblerUsed);
  if (gam && cycle >= 2 && chance(0.5)) {
    ...
    if (prey.length) { if (chance(0.79)) killPlayer(pick(prey), players); else killPlayer(gam, players); }
  }
...
function simulateGame(n) {
  ...
  for (let cycle = 1; cycle <= P.maxCycles && !win; cycle++) {
    ...
    win = evaluateWin(players); if (win) break;

    // ENQUÊTE + VOTE → PRISON
    runInvestigations(players, cycle);
    const lynched = runVote(players);
    if (lynched) {
      if (lynched.role.saintTrap) { win = { winner: "Méchants", saint: true }; break; }
...
function classifyWinner(win, players) {
  ...
  if (["veuve_noire", "parieur_tricheur", "empoisonneur", "conservateur", "heritier_dechu"].includes(w)) return "Neutre:" + w;
  ...
  for (let g = 0; g < games; g++) {
    ...
    for (const p of players) {
      ...
      switch (os) {
        ...
        case "empoisonneur": won = winner === "empoisonneur"; break;
        case "veuve_noire": won = winner === "veuve_noire"; break;
        case "parieur_tricheur": won = winner === "parieur_tricheur"; break;
        case "conservateur": won = winner === "conservateur"; break;
        case "heritier_dechu": won = winner === "Méchants" && p.alive && p.free; break; // gagne AVEC les méchants
        case "oracle": won = p.alive && p.free && p.prediction === winFaction; break;
        case "imitateur":
          if (p.transformed) {
            won = (winner === "Civil" && p.faction === "Civil") ||
...
if (!_isMain) { /* importé : ne lance pas le main */ }
...
for (const n of SIZES) {
  ...
  for (const k of Object.keys(neutreWinWhenPresent)) gWin[k] = (gWin[k] || 0) + neutreWinWhenPresent[k];
  const civ = tally["Civils"] || 0;
  const mech = tally["Méchants"] || 0;
  const neu = Object.entries(tally).filter(([k]) => k.startsWith("Neutre")).reduce((s, [, v]) => s + v, 0);
  const to = tally["Timeout"] || 0;
  globalFaction.Civils += civ; globalFaction.Méchants += mech; globalFaction.Neutre += neu; globalFaction.Timeout += to; globalFaction.total += GAMES;
  ...
  console.log(`──────── ${n} JOUEURS  (compo type: ${cc} civils / ${mc} méchants / ${nc} neutres) ────────`);
  console.log(`  Civils   : ${(100 * civ / GAMES).toFixed(1)}%`);
  console.log(`  Méchants : ${(100 * mech / GAMES).toFixed(1)}%`);
  console.log(`  Neutres  : ${(100 * neu / GAMES).toFixed(1)}%   (timeout ${(100 * to / GAMES).toFixed(1)}%)`);
  // ratio civ/méchant hors neutres
  const cm = civ + mech;
  if (cm > 0) console.log(`  → Civils vs Méchants (parties tranchées): ${(100 * civ / cm).toFixed(1)}% / ${(100 * mech / cm).toFixed(1)}%`);
  // détail neutres
  const neuDetail = Object.entries(tally).filter(([k]) => k.startsWith("Neutre")).sort((a, b) => b[1] - a[1]);
  if (neuDetail.length) {
    console.log("  Neutres gagnants (part des parties):");
    for (const [k, v] of neuDetail) console.log(`     ${k.replace("Neutre:", "").padEnd(18)} ${(100 * v / GAMES).toFixed(2)}%`);
  }
  ...
  for (const slug of Object.keys(present).sort()) {
    const pr = present[slug], wn = neutreWinWhenPresent[slug] || 0;
    console.log(`     ${slug.padEnd(18)} présent ${(100 * pr / GAMES).toFixed(0).padStart(3)}% des parties · win ${(100 * wn / pr).toFixed(1)}%`);
  }
  console.log("");
}
...
console.log(`  → Civils/Méchants seuls : ${(100 * globalFaction.Civils / CM).toFixed(1)}% / ${(100 * globalFaction.Méchants / CM).toFixed(1)}%  (cible 55/45)`);

console.log("\n════════ WIN-RATE NEUTRES — quand présent (toutes tailles) ════════");
const order = Object.keys(gPresent).sort((a, b) => (gWin[b] || 0) / gPresent[b] - (gWin[a] || 0) / gPresent[a]);
for (const slug of order) {
  const pr = gPresent[slug], wn = gWin[slug] || 0;
  console.log(`  ${slug.padEnd(18)} présent ${(100 * pr / T).toFixed(0).padStart(3)}% · win ${(100 * wn / pr).toFixed(1).padStart(5)}%`);
}
} // fin _isMain
