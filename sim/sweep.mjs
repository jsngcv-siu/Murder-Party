...
const killValues = [0.5, 0.6, 0.7, 0.8];
const coverValues = [0.45, 0.7, 0.9];

// Table 1 : reliability × killerSuccess (cover fixé baseline 0.45)
console.log(
  "TABLE 1 — précision d'enquête (lignes) × létalité Tueur (colonnes), cover malus = 0.45",
);
...
for (const cv of coverValues) {
  const v = civShare({
    ...
    killerSuccessBase: 0.7,
    coverReliabilityMalus: cv,
    townCoordination: 0.75,
  });
