...
import { executeCapability, whyCannotUse, usesOf, parseTotalLimit, type CapabilityResult, type RoleRow } from "@/engine/actions";
...
import {
  Axe, Ban, Bird, Check, ChefHat, CircleCheck, CircleHelp, Crown, Dices, Drama, Droplet, Eraser, Eye,
  FlaskConical, Heart, History, Hourglass, Info, KeyRound, Lock, LockOpen, MessageCircle, Moon,
  Network, NotebookPen, Orbit, Radar, Scale, ScrollText, Search, Shield, ShieldCheck, Skull,
  Sparkles, Stethoscope, Swords, Target, TriangleAlert, Trophy, Zap, type LucideIcon,
} from "lucide-react";
...
function SaintBlockedLog({ gameId, meId }: { gameId: string; meId: string }) {
  const [logs, setLogs] = useState<Array<{ id: string; title: string; body: string | null; created_at: string }>>([]);
  useEffect(() => {
    let cancelled = false;
    async function load() {
      const { data } = await supabase
        ...
        .limit(20);
      if (!cancelled) setLogs((data ?? []) as Array<{ id: string; title: string; body: string | null; created_at: string }>);
    }
    void load();
    const ch = supabase
      .channel(`saint-log-${gameId}-${meId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "notifications", filter: `game_id=eq.${gameId}` }, () => void load())
      .subscribe();
  ...
  return (
    <div className="mt-2 space-y-1">
      <div className="text-[10px] uppercase tracking-wider text-yellow-300/70">Actions annulées</div>
      {logs.map((l) => (
        <div key={l.id} className="rounded bg-yellow-500/10 border border-yellow-400/30 px-2 py-1 text-[11px] text-yellow-100">
          {l.body ?? l.title}
        </div>
      ))}
    </div>
...
export function actionSummary(payload: Record<string, unknown>, fallbackResult?: Record<string, unknown>): string {
  // Priorité : le message humain stocké après exécution.
  const msg = (fallbackResult?.message as string | undefined) ?? (payload.message as string | undefined);
  if (msg) return msg;
  const summary = (fallbackResult?.summary as string | undefined) ?? (payload.summary as string | undefined);
...
const SUCCESS_RX = /\b(réussi|succès|soigné|protégé|sauvé|tué|empoisonné|empris|condamn|révélé|découvert|confirmé|trouvé|identifié|volé|hérit|libéré|mordu|chargé|verrouill|enregistr|posté|placé|distribu|confiée?)\b/i;
const FAIL_RX = /\b(échec|raté|rien|aucun|bloqué|impossible|introuvable|épuisée?|hors phase|refusé|annulé|falsifi)\b/i;
...
function parseResultMessage(raw: string): ParsedResult {
  ...
  if (m) { icon = m[1]; body = text.slice(m[0].length).trim(); }
  let headline = body;
  let details = "";
...
export function ResultBlock({
  raw, roles, size = "md", outcome, verdict = false,
}: { raw: string; roles: Map<string, RoleRow>; size?: "sm" | "md"; outcome?: ResultTone; verdict?: boolean }) {
  ...
  const toneAccent =
    tone === "success" ? "oklch(0.78 0.18 145)" :
    tone === "fail" ? "oklch(0.70 0.20 25)" :
    tone === "pending" ? "oklch(0.80 0.15 70)" :
    "oklch(0.75 0.10 250)";
  const toneBadgeBg =
    tone === "success" ? "oklch(0.78 0.18 145 / 0.14)" :
    tone === "fail" ? "oklch(0.70 0.20 25 / 0.14)" :
    tone === "pending" ? "oklch(0.80 0.15 70 / 0.14)" :
    "oklch(0.75 0.10 250 / 0.14)";
  const toneLabel = tone === "success" ? "Réussi" : tone === "fail" ? "Échec" : tone === "pending" ? "En cours" : "Info";
  ...
  const ToneIcon: LucideIcon = tone === "success" ? CircleCheck : tone === "fail" ? TriangleAlert : tone === "pending" ? Hourglass : Info;
  ...
  return (
    <div className="flex items-start gap-2">
      {msgEmoji
        ? <span className="shrink-0 text-xl leading-none mt-0.5" aria-hidden>{msgEmoji}</span>
...
function capabilityState(
  ...
  if (mode === "none")
    return { label: "Capacité passive", Icon: Moon, cls: "text-muted-foreground bg-muted/40 border-border", hint: "Automatique — rien à faire de ta part." };
  if (!blockedReason)
    return { label: "À utiliser ce tour", Icon: Zap, cls: "text-gold bg-gold/12 border-gold/40", hint: "C'est le moment d'agir · le dénouement est révélé à l'Annonce." };
  if (/^Déjà utilisé/i.test(blockedReason))
    return { label: "Action faite", Icon: Check, cls: "text-emerald-300 bg-emerald-500/15 border-emerald-500/40", hint: "C'est joué · rendez-vous à l'Annonce." };
  if (/épuisée/i.test(blockedReason))
    return { label: "Capacité épuisée", Icon: Ban, cls: "text-muted-foreground bg-muted/40 border-border", hint: "Plus aucune utilisation disponible." };
  if (/cooldown/i.test(blockedReason))
    return { label: "En récupération", Icon: Hourglass, cls: "text-amber-300 bg-amber-500/12 border-amber-500/40", hint: "Bientôt de nouveau disponible." };
  // Autres motifs (mauvaise phase, Stratège…) : on affiche le motif tel quel.
  return { label: blockedReason, Icon: Hourglass, cls: "text-amber-300 bg-amber-500/12 border-amber-500/40", hint: "Indisponible pour l'instant." };
}
...
function deriveOutcome(r: { ok: boolean; pending?: boolean; reveal?: Record<string, unknown> }, role: RoleRow | null): ResultTone {
  if (!r.ok) return "fail";
...
type PanelTone =
  | "fuchsia" | "violet" | "purple" | "indigo" | "sky" | "amber"
  | "yellow" | "rose" | "pink" | "emerald" | "red" | "neutral";

function PanelCard({
  tone = "neutral", icon: Icon, label, action, children, className = "",
}: {
  tone?: PanelTone; icon?: LucideIcon; label?: string;
      ...
      {(label || Icon || action) && (
        <div className="flex items-center justify-between gap-2 mb-2">
          <span
            ...
            style={{ color: accent }}
          >
            {Icon && <Sigil active size={18} accent={accent}><Icon className="size-3.5" aria-hidden /></Sigil>}
            {label}
          </span>
          {action}
        </div>
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      <nav className="grid grid-cols-4 gap-1.5 p-2 border-b border-border bg-gradient-to-b from-card/60 to-card/20">
        <TabBtn active={tab === "role"} onClick={() => setTab("role")} icon={<Drama className="size-5" />} label="Rôle" accent="oklch(0.74 0.15 300)" />
        <TabBtn active={tab === "history"} onClick={() => setTab("history")} icon={<ScrollText className="size-5" />} label="Historique" accent="var(--citoyens)" />
        <TabBtn active={tab === "win"} onClick={() => setTab("win")} icon={<Trophy className="size-5" />} label="Victoire" accent="var(--primary)" />
        <TabBtn
          active={tab === "chat"}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {tab === "history" && <HistoryTab gameId={gameId} meId={me.id} players={players} roles={roles} />}
        {tab === "win" && <WinTab me={me} myRole={myRole} players={players} />}
        {tab === "chat" && chatAvailable && (
          <ChatTab gameId={gameId} meId={me.id} mePseudo={me.pseudo} isMedium={!!isMedium} isDead={!me.is_alive} />
        )}
      </div>
    </div>
  );
...
function TabBtn({ active, onClick, icon, label, disabled, accent }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string; disabled?: boolean; accent?: string }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
    ...
    >
      ...
      <span className="text-[10px] uppercase tracking-wider font-semibold leading-none" style={active && accent ? { color: accent } : undefined}>{label}</span>
    </button>
  );
}
...
function RoleTab({ ctx }: { ctx: FrameContext }) {
  ...
  const blockedReason = myRole ? whyCannotUse(myRole, meMeta, game.current_tour, playerCount, game.current_phase as "free" | "gathering" | "vote") : null;
  ...
  const rawStatusBlockReason: string | null =
    (((meMeta.blackmail_until_cycle as number | undefined) ?? -1) >= tour && ((meMeta.blackmail_from_cycle as number | undefined) ?? -Infinity) <= tour) ? "Sous chantage"
    : (((meMeta.blocked_until_cycle as number | undefined) ?? -1) >= tour && ((meMeta.blocked_from_cycle as number | undefined) ?? -Infinity) <= tour) ? "Capacité bloquée"
    : (((meMeta.drunk_until_cycle as number | undefined) ?? -1) >= tour && ((meMeta.drunk_from_cycle as number | undefined) ?? -Infinity) <= tour) ? "Ivre — servi par le barman"
    : null;
  // Si le Marionnettiste pilote cette capacité, on ignore le blocage qu'il a lui-même posé.
  const statusBlockReason = puppeteerOverride && rawStatusBlockReason === "Capacité bloquée" ? null : rawStatusBlockReason;
  ...
  const mode = (myRole?.target_mode ?? "single") as "none" | "single" | "double" | "multi" | "self_or_other";
  const needed = mode === "double" ? 2 : mode === "multi" ? 3 : mode === "none" ? 0 : 1;
  const minTargets = mode === "double" ? 2 : needed;
  const aliveOthers = players.filter((p) => p.is_alive && !p.is_imprisoned && !p.is_mj && p.id !== me.id);
  ...
  useEffect(() => {
    ...
    mm.add({ ok: "(prefers-reduced-motion: no-preference)" }, () => {
      gsap.from(".role-zone", { autoAlpha: 0, y: 8, scale: 0.98, duration: 0.45, ease: "back.out(1.4)" });
    }, el);
    return () => mm.revert();
  }, [myRole?.slug]);
  ...
  ].filter((f) => !flasksUsed.includes(f.id)) as Array<{ id: "heal" | "poison" | "reveal"; label: string; Icon: LucideIcon }>;

  const allies = isMechantTeam
    ? players.filter((p) => !p.is_mj && p.id !== me.id && (() => {
  ...
  async function useCapacity(overrides?: { extra?: Record<string, unknown>; skipTargetCheck?: boolean; targetIds?: string[] }) {
    if (busy) return;
    const effectiveTargets = overrides?.targetIds ?? targets;
    if (!overrides?.skipTargetCheck && mode !== "none" && effectiveTargets.length < minTargets) return;
    setBusy(true);
    ...
    const r = await executeCapability({
      gameId, actor: me, role: myRole, targets: targetRows,
      tour: game.current_tour,
      phase: game.current_phase as "free" | "gathering" | "vote",
      allPlayers: players, rolesBySlug: roles,
      extra,
    });

    setResult(r);
    if (r.ok && r.message) {
      const { data: last } = await supabase.from("role_actions")
        .select("id")
        .eq("game_id", gameId).eq("actor_player_id", me.id)
        .eq("tour", game.current_tour).eq("phase", game.current_phase as "free" | "gathering" | "vote")
        .order("created_at", { ascending: false }).limit(1).maybeSingle();
      const lastId = (last as { id: string } | null)?.id;
      if (lastId) {
        await supabase.from("role_actions")
          .update({
            result: {
              summary: r.message,
      ...
      <div className="p-5 flex flex-col">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Ton rôle</div>
  ...
  const showGenericUseBtn =
    mode !== "none" && !isOraclePending &&
    myRole?.slug !== "cuisinier" && myRole?.slug !== "vengeur" &&
    myRole?.slug !== "juge" && myRole?.slug !== "executeur" &&
  ...
  return (
    <div ref={rootRef} className={`p-5 flex flex-col ${hasStickyFooter ? "pb-40" : ""}`}>
      ...
      {(() => {
        ...
        const factionColor = f === "Civil" ? "var(--citoyens)" : f === "Méchant" ? "var(--destructive)" : "var(--neutres)";
          ...
          <div className="role-zone">
            <div className="flex items-center gap-3.5">
              <div
                ...
                style={{ boxShadow: `0 0 0 1.5px color-mix(in oklab, ${factionColor} 55%, transparent), 0 0 18px -5px ${factionColor}` }}
              >
                <RoleIcon role={myRole} size={52} />
              </div>
              <div className="flex-1 min-w-0">
                <h2 className="text-xl font-bold leading-tight text-glow-gold" style={{ fontFamily: "var(--font-display)" }}>
                  {myRole?.name_fr ?? "—"}
                </h2>
                <div className="mt-1.5 flex items-center gap-1.5 flex-wrap">
                  ...
                  {freqs.map((f, i) => (
                    <span key={i} className="text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded border border-gold/40 bg-gold/10 text-gold">{f}</span>
                  ))}
                  {origRole && (
                    <span className="text-[10px] font-normal text-muted-foreground">ex {origRole.name_fr}</span>
                  )}
                </div>
              </div>
            </div>
        ...
        return (
          <div className={`mt-4 rounded-xl border p-3 flex items-start gap-3 ${st.cls}`}>
            ...
            <div className="min-w-0 flex-1">
              ...
              {st.hint && <div className="text-[11px] opacity-80 mt-0.5 leading-snug">{st.hint}</div>}
            </div>
          </div>
        );
      })()}
      ...
      {isMechantTeam && (
        <div className="mt-2 rounded-lg border border-destructive/30 bg-destructive/5 p-2 text-xs space-y-1">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1">Liste de tes alliés</div>
          {allies.length > 0 ? (
            allies.map((a) => {
              const r = roles.get(a.role_slug ?? "");
              const aMeta = (a.role_meta ?? {}) as Record<string, unknown>;
              const originalSlug = typeof aMeta.original_slug === "string" ? aMeta.original_slug : null;
              ...
              return (
                <div key={a.id} className="flex items-center gap-2">
                  ...
                  <span className="text-muted-foreground">— {roleLabel}{extra}</span>
                </div>
              );
            })
          ) : (
      ...
      {/* Statuts affichés uniquement dans la barre du haut (StatusBandeau) */}
      {(() => {
        const partnerId = (meMeta.beloved_id as string | undefined) ?? (meMeta.etre_cher as string | undefined) ?? (meMeta.lover_id as string | undefined) ?? null;
        ...
        return (
          <div className="mt-3 rounded-lg border border-pink-400/40 bg-gradient-to-r from-pink-500/10 to-rose-500/10 px-3 py-2 text-xs text-pink-200 flex items-center gap-2">
            ...
            <span>Lien avec <span className="font-semibold text-pink-100">{partner.pseudo}</span>{isLover ? ". Si l'un meurt, l'autre suit. Vous gagnez ensemble." : "."}</span>
          </div>
        );
      })()}

      {myRole?.slug === "temoin" && <WitnessRevealPanel meId={me.id} gameId={gameId} players={players} roles={roles} />}
      {myRole?.slug === "voisin" && (
        <NeighborWatchPanel
          gameId={gameId}
      ...
      {myRole?.slug === "guetteur" && (
        <GuetteurVisitsPanel gameId={gameId} meId={me.id} players={players} roles={roles} tour={game.current_tour} />
      )}
      {myRole?.slug === "paranoiaque" && (
        <ParanoiaquePanel
          ...
          onAct={(choice) => void useCapacity({ extra: { choice }, skipTargetCheck: true })}
        />
      )}
      {myRole?.slug === "medecin_legiste" && (
        <LegisteAutopsiesPanel gameId={gameId} meId={me.id} />
      )}
      {myRole?.slug === "avocat" && (
        <AvocatPrisonPanel players={players} roles={roles} />
      )}
      {isOraclePending && (
        <PanelCard tone="fuchsia" icon={Sparkles} label="Prophétie — choisis le camp gagnant">
          ...
          <button
            ...
            onClick={() => useCapacity({ extra: { faction: oracleFaction }, skipTargetCheck: true })}
      ...
      {myRole?.slug === "usurpateur" && !meMeta.cover_slug && (
        <UsurpateurCoverChoice
          meId={me.id}
          meMeta={meMeta}
          roles={roles}
        />
      )}
      {myRole?.slug === "usurpateur" && typeof meMeta.cover_slug === "string" && (() => {
        const r = roles.get(meMeta.cover_slug as string);
        return r ? (
          <PanelCard tone="red" icon={Drama} label="Couverture verrouillée">
            <span className="font-semibold inline-flex items-center gap-1"><RoleIcon role={r} size={16} /> {r.name_fr}</span>. Toutes les enquêtes te désigneront ainsi.
          </PanelCard>
        ) : null;
      })()}
      {myRole?.slug === "imitateur" && !meMeta.imitated_slug && (
        <PanelCard tone="amber" icon={Drama} label="Imitation">
          ...
          {imitateurLastDead ? ` : ${imitateurLastDead.pseudo}.` : " — mais personne n'est encore mort."}
        </PanelCard>
      )}
      {myRole?.slug === "imitateur" && typeof meMeta.imitated_slug === "string" && (() => {
        ...
        return r ? (
          <PanelCard tone="amber" icon={Drama} label="Imitation active" className="space-y-1">
            <div>Tu imites désormais : <span className="font-semibold inline-flex items-center gap-1"><RoleIcon role={r} size={16} /> {r.name_fr}</span>.</div>
      ...
      {myRole?.slug === "heritier_dechu" && (
        <PanelCard tone="yellow" icon={Crown} label="Héritier déchu">
          À chaque phase libre, désigne une cible : tu apprends si elle <span className="font-semibold">éveille les soupçons</span> ou non. Tu gagnes en vie avec les Méchants à la fin de la partie.
        </PanelCard>
      )}
        ...
        <PanelCard tone="amber" icon={Dices} label="Le pari truqué">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1 shrink-0">
              <MiniDie value={6} /><MiniDie value={5} />
            </span>
            <div className="text-amber-100/90 leading-snug">
              Tu lances <span className="font-bold">2 dés à 6 faces</span> et gardes le <span className="font-bold">meilleur</span> ; ta cible n'en lance qu'<span className="font-bold">un</span>. Le plus petit nombre meurt à la prochaine annonce — <span className="font-semibold">égalité&nbsp;: on relance</span>.
            </div>
          </div>
      ...
      {(myRole?.slug === "vampire" || meMeta.converted === true) && (() => {
        const clan = players.filter((p) => p.is_alive && p.id !== me.id && (p.role_slug === "vampire" || (p.role_meta as Record<string, unknown> | null)?.converted === true));
        return (
          <PanelCard tone="red" icon={Droplet} label="Ton clan vampire">
            {meMeta.converted === true && (
              <div className="text-[11px] text-red-300/80 italic mb-1">Morsure subie — tu rejoins les Vampires.</div>
            )}
            {clan.length === 0 ? (
              <div className="text-red-300/70 italic">Personne d'autre pour l'instant. Mords pour agrandir le clan.</div>
            ) : (
              <div className="flex flex-wrap gap-1">
                {clan.map((c) => (
                  <span key={c.id} className="px-2 py-0.5 rounded bg-red-500/15 border border-red-400/30">{c.pseudo}</span>
                ))}
              </div>
            )}
          </PanelCard>
      ...
      {myRole?.slug === "empoisonneur" && (() => {
        ...
        const poisoned = others.filter((p) => p.is_alive && (p.role_meta as Record<string, unknown> | null)?.poisoned === true);
        const remaining = others.filter((p) => p.is_alive && !p.is_imprisoned && (p.role_meta as Record<string, unknown> | null)?.poisoned !== true);
        return (
          <PanelCard tone="emerald" icon={FlaskConical} label="Suivi d'empoisonnement" action={<span className="text-[10px] uppercase tracking-wider text-emerald-300/80">Live</span>}>
            ...
            <div className="mb-2">
              ...
              {poisoned.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-1">
                  {poisoned.map((p) => (
                    <span key={p.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-500/20 border border-emerald-400/40">
                      {p.pseudo}{p.is_imprisoned && <Lock className="size-2.5" aria-hidden />}
                    </span>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              {remaining.length === 0 ? (
                <div className="text-emerald-200 font-semibold inline-flex items-center gap-1.5"><Trophy className="size-4" aria-hidden /> Tous les survivants libres sont empoisonnés !</div>
              ) : (
                <div className="flex flex-wrap gap-1">
                  {remaining.map((p) => (
                    <span key={p.id} className="px-2 py-0.5 rounded bg-amber-500/15 border border-amber-400/40 text-amber-200">
                      {p.pseudo}
                    </span>
                  ))}
                </div>
      ...
      {myRole?.slug === "marionnettiste" && (
        <MarionnettistePanel ctx={ctx} />
      )}

      {myRole?.slug === "saint" && meMeta.saint_used === true && (() => {
        const tgtId = meMeta.saint_target_id as string | undefined;
        const tgtPseudo = (meMeta.saint_target_pseudo as string | undefined) ?? players.find((p) => p.id === tgtId)?.pseudo ?? "?";
        return (
          <PanelCard tone="yellow" icon={Sparkles} label="Sainte bénédiction">
            <div className="mb-1 text-sm">Tu protèges : <span className="text-yellow-50 font-bold">{tgtPseudo}</span></div>
            <div className="text-[11px] text-yellow-200/80 italic">
              La bénédiction annule toute action malveillante (méchants & neutres MAL/CHAOS) pendant 2 tours complets.
            </div>
        ...
        return (
          <div className="mt-3 rounded-xl border-2 border-purple-400/60 bg-gradient-to-br from-purple-600/25 to-fuchsia-600/15 px-4 py-3 text-sm text-purple-100 shadow-[0_0_20px_rgba(168,85,247,0.25)]">
            ...
            <div className="text-purple-200/90 text-xs">
              {forcedTarget
                ? <>Au prochain rassemblement, tu frapperas <span className="font-semibold text-purple-50">{forcedTarget.pseudo}</span> sans le vouloir.</>
      ...
      {myRole?.slug === "ange_gardien" && (() => {
        const wardId = (meMeta.ward as string | undefined) ?? (meMeta.protege_id as string | undefined);
        ...
        return (
          <PanelCard tone="emerald" icon={ShieldCheck} label="Cible à protéger">
            {wardPseudo ? (
              <div className="flex items-center gap-3">
                {ward && <AvatarImg avatar={playerAvatar(ward)} size={40} className="ring-1 ring-emerald-500/30" />}
                <div className="flex-1 min-w-0">
      ...
      {!NO_LAST_RESULT_ROLES.has(myRole?.slug ?? "") && (
        <LastResultBanner gameId={gameId} meId={me.id} players={players} roles={roles} currentTour={game.current_tour} myRoleSlug={myRole?.slug} />
      )}
      ...
      {myRole?.slug === "juge" && (() => {
        ...
        return (
          <JugePrisonPanel
            ...
            onRelease={(id) => void useCapacity({ targetIds: [id], skipTargetCheck: true })}
          />
        );
      })()}
      {myRole?.slug === "executeur" && (() => {
        ...
        return (
          <ExecuteurPrisonPanel
            ...
            remaining={remaining}
            total={total === Infinity ? null : total}
            onExecute={(id) => void useCapacity({ targetIds: [id], skipTargetCheck: true })}
          />
        );
      })()}
      ...
      {isApothicaire && (
        <div className="mt-3">
          <div className="rounded-lg border border-amber-400/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-200 mb-2">
            Choisis une fiole à <span className="font-semibold">offrir</span> à un joueur : il l'utilisera comme il veut depuis son Carnet.
            Tu dois en offrir au moins 2 sur 3. Pour en <span className="font-semibold">garder une</span> et l'utiliser toi-même, passe par ton Carnet (1 maximum).
          </div>
          <div className="flex items-center justify-between text-[10px] uppercase tracking-wider text-muted-foreground mb-1">
            ...
            <span>Offertes : {(meMeta.fioles_given as number | undefined) ?? 0} · Gardée : {(meMeta.fioles_self_used as number | undefined) ?? 0}/1</span>
          </div>
          <div className="grid grid-cols-3 gap-2">
            {fiolesAvail.length === 0 && (
              <div className="col-span-3 text-xs text-muted-foreground italic">Toutes les fioles ont été utilisées.</div>
            )}
            {fiolesAvail.map((f) => (
              <button
                key={f.id}
      ...
      {myRole?.slug === "cuisinier" && (
        <div className="mt-3 rounded-lg border border-amber-400/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-200 flex items-start gap-1.5">
          <ChefHat className="size-4 shrink-0 mt-0.5" aria-hidden /> <span>Capacité passive — tu as reçu un couteau au début de la partie. Utilise-le depuis ton inventaire.</span>
        </div>
      )}
      {myRole?.slug === "vengeur" && !meMeta.etre_cher && !meMeta.beloved_id && (
        <VengeurBelovedChoice meId={me.id} players={players} roles={roles} />
      )}
      {myRole?.slug === "vengeur" && Boolean(meMeta.etre_cher || meMeta.beloved_id) && (() => {
        const belovedId = (meMeta.beloved_id as string | undefined) ?? (meMeta.etre_cher as string | undefined);
        ...
        return (
          <div className="mt-3 rounded-lg border border-pink-400/40 bg-pink-500/10 px-3 py-2 text-xs text-pink-200 flex items-start gap-1.5">
            ...
            <span>Être cher verrouillé : <span className="font-semibold">{beloved?.pseudo ?? "?"}</span>.
            {unlocked
              ? <span className="inline-flex items-center gap-1"> <Swords className="size-3.5" aria-hidden /> Vengeance débloquée — utilise ton couteau depuis l'inventaire.</span>
              : " Tu recevras un couteau si il/elle meurt."}
            </span>
          </div>
        );
      })()}
      {mode !== "none" && !isOraclePending && myRole?.slug !== "cuisinier" && myRole?.slug !== "vengeur" && myRole?.slug !== "juge" && myRole?.slug !== "executeur" && myRole?.slug !== "paranoiaque" && !(myRole?.slug === "saint" && meMeta.saint_used === true) && !(blockedReason != null && /^Déjà utilisé/.test(blockedReason)) && (
        <>
          <div className="mt-3 text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
            {myRole?.instruction_verb ?? "Cible"} {needed > 1 ? `(${targets.length}/${needed})` : ""}
          </div>
            ...
            {targetable.map((p, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`flex-1 truncate font-semibold ${sel ? "text-foreground" : "text-foreground/90"}`}>
                  ...
                  {sel ? (
                    ...
                    <span className="shrink-0 text-lg text-muted-foreground leading-none">{isLeft ? "›" : "‹"}</span>
                  )}
                </button>
              );
            })}
      ...
      {myRole?.slug === "cleaner" && (() => {
        ...
        return (
          <PanelCard tone="neutral" icon={Eraser} label="Cleaner" action={
            <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-2 py-0.5 rounded-full ${armed ? "bg-emerald-500/20 text-emerald-300 ring-1 ring-emerald-500/40" : "bg-muted text-muted-foreground"}`}>
              <span className={`size-2 rounded-full ${armed ? "bg-emerald-400" : "bg-muted-foreground/60"}`} aria-hidden />
              {armed ? "ARMÉ" : "INACTIF"}
            </span>
          }>
            ...
            <div className="mt-2.5 flex items-center justify-between rounded-lg border border-border bg-background/60 px-3 py-2">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Utilisations restantes</span>
              <span className={`text-sm font-bold ${exhausted ? "text-destructive" : "text-foreground"}`}>
      ...
      {myRole?.slug === "conservateur" && (
        <div className="mt-3 rounded-lg border border-amber-400/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-200 flex items-start gap-1.5">
          <KeyRound className="size-4 shrink-0 mt-0.5" aria-hidden /> <span>Désigne un joueur ci-dessous : il recevra une relique maudite au hasard.</span>
        </div>
      )}
      ...
      {hasStickyFooter && (
        <div className="sticky bottom-0 left-0 right-0 -mx-5 -mb-5 mt-4 px-5 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] bg-gradient-to-t from-background via-background/95 to-background/80 backdrop-blur-md border-t border-border z-20 space-y-2 shadow-[0_-8px_24px_-12px_rgba(0,0,0,0.6)]">
          ...
          {showGenericUseBtn && (
            <button
              disabled={
                busy || !!blockedReason || targets.length < minTargets ||
                (isApothicaire && fiolesAvail.length === 0)
              }
              onClick={() => void useCapacity()}
              style={{
                WebkitTapHighlightColor: "transparent",
            ...
            >
              {busy ? "…" : isApothicaire ? `Offrir la fiole${targets.length > 0 ? ` à 1 joueur` : ""}` : `Utiliser${targets.length > 0 ? ` (${targets.length}/${needed})` : ""}`}
            </button>
          )}
          {showAngeGardienBtn && (
            <button
            ...
            >
              {busy ? "…" : <span className="inline-flex items-center gap-1.5"><Shield className="size-4" aria-hidden /> Activer le bouclier</span>}
            </button>
          )}
        </div>
      )}
  ...
  if (puppetActiveTour > game.current_tour) {
    return (
      <PanelCard tone="red" icon={Drama} label="Marionnette préparée">
        <div>
          Au prochain tour, tu prendras le contrôle de <span className="font-semibold">{puppet.pseudo}</span>.
        </div>
      </PanelCard>
    );
  }
  ...
  if (puppetActiveTour < game.current_tour) {
    return (
      <PanelCard tone="neutral" icon={Drama} label="Manipulation terminée">
        <div>Manipulation de <span className="font-medium">{puppet.pseudo}</span> terminée.</div>
      </PanelCard>
    );
  }
    ...
    <>
      <PanelCard tone="red" icon={Drama} label="Manipulation en cours">
        <div className="text-purple-200/90 text-xs mb-3">
          ...
          {puppetRole ? <> — rôle : <span className="font-semibold">{puppetRole.name_fr}</span></> : null}.
        ...
        >
          <span className="inline-flex items-center gap-1.5"><Eye className="size-4" aria-hidden /> Voir l'onglet capacité du manipulé</span>
        </button>
      </PanelCard>
      {open && (
        <div
          ...
          >
            <div className="flex items-center justify-between p-3 border-b border-purple-400/30 bg-purple-500/10">
              <div className="text-xs">
                <div className="uppercase tracking-wider text-purple-300 inline-flex items-center gap-1.5"><Drama className="size-3.5" aria-hidden /> Capacité pilotée</div>
                <div className="font-semibold text-purple-50">{puppet.pseudo}{puppetRole ? ` — ${puppetRole.name_fr}` : ""}</div>
              </div>
              <button
                onClick={() => setOpen(false)}
...
function FalsificateurPanel({
  ...
  return (
    <PanelCard
      ...
      action={<span className="text-[10px] px-1.5 py-0.5 rounded-md bg-destructive/15 text-destructive font-semibold tabular-nums">{falsified.length}</span>}
    >
      {falsified.length === 0 ? (
        <div className="text-[11px] text-muted-foreground italic">Aucun joueur falsifié pour l'instant.</div>
      ) : (
        <div className="space-y-1.5">
          {falsified.map(({ p, tour }) => {
            ...
            const r = roles.get(p.role_slug ?? "");
            return (
              <div key={p.id} className="flex items-center gap-2 rounded-lg bg-background/40 border border-border/40 px-2 py-1.5">
                <AvatarImg id={p.id} avatar={avatarOf(meta.avatar as string | undefined, p.id)} size={26} />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-foreground/95 truncate">{p.pseudo}</div>
                  {r && <div className="text-[10px] text-muted-foreground truncate inline-flex items-center gap-1"><RoleIcon role={r} size={12} /> {r.name_fr}</div>}
                </div>
                {tour != null && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-fuchsia-500/10 text-fuchsia-200/80 font-semibold tabular-nums">T{tour}</span>
                )}
              </div>
            );
          })}
...
const NO_LAST_RESULT_ROLES = new Set<string>([
  "temoin", "voisin", "usurpateur",
  "avocat", "guetteur", "cuisinier", "juge",
  "medecin_legiste", "medium",
  "cartomancien", "journaliste", "veuve_noire",
  "paranoiaque",
  "oracle", "entremetteur", "imitateur", "vengeur",
  "ange_gardien", "saint",
  // Décision joueur : pas de bandeau (panneau dédié ou pas d'info utile)
  "cleaner", "marionnettiste", "falsificateur", "empoisonneur",
  "vampire", "conservateur",
]);

// ───────── Bannière persistante : Dernier résultat de capacité
type LastRow = {
  id: string; tour: number; phase: string;
...
const ACTION_DESCRIPTIONS: Record<string, (t1?: string, t2?: string) => string> = {
  ...
  entremetteur: (t, t2) => t && t2 ? `Tu as lié ${t} et ${t2}.` : `Tu as tenté de lier deux joueurs.`,
  executeur: (t) => `Tu as désigné ${t ?? "un joueur"} comme cible.`,
  facteur: (t) => `Tu as posté un message à ${t ?? "un joueur"}.`,
  falsificateur: (t) => t ? `Tu as falsifié des informations visant ${t}.` : `Tu as falsifié des informations.`,
  ...
  marionnettiste: (t, t2) => t && t2 ? `Tu as manipulé ${t} et ${t2}.` : `Tu as manipulé ${t ?? "un joueur"}.`,
...
function LastResultBanner({
  gameId, meId, players, roles, currentTour, myRoleSlug,
}: {
  gameId: string; meId: string;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data } = await supabase.from("role_actions")
        ...
        .eq("game_id", gameId).eq("actor_player_id", meId)
        .eq("tour", currentTour)
        .order("created_at", { ascending: false }).limit(1).maybeSingle();
    ...
    void load();
    const ch = supabase.channel(`last-result-${meId}-${currentTour}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "role_actions", filter: `actor_player_id=eq.${meId}` }, (p) => {
        const row = p.new as LastRow;
        if (!row || row.tour !== currentTour) return;
        setLast((prev) => (prev && prev.id === row.id ? prev : row));
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "role_actions", filter: `actor_player_id=eq.${meId}` }, (p) => {
        const row = p.new as LastRow;
        if (!row || row.tour !== currentTour) return;
        setLast((prev) => (prev && prev.id === row.id ? { ...prev, payload: row.payload ?? prev.payload, result: row.result ?? prev.result } : prev));
      })
      .subscribe();
    return () => { cancelled = true; void supabase.removeChannel(ch); };
  ...
  const hasOutcome = !!resultMsg && resultMsg !== "Capacité utilisée." && resultMsg.trim() !== actionText.trim();
  // Ton fiable depuis result.outcome (persisté), sinon repli regex.
  const storedOutcome = (last.result as Record<string, unknown> | null)?.outcome as ResultTone | undefined;
  const tone: ResultTone = storedOutcome ?? (hasOutcome ? parseResultMessage(resultMsg).tone : "info");
  ...
  return (
    <div className="mt-3">
      <div className="flex items-center justify-between px-1 pb-1.5">
        <span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Résultat</span>
        <span className="text-[10px] text-muted-foreground tabular-nums">T{last.tour} · {time}</span>
      </div>
      ...
      >
        ...
        {hasOutcome ? (
          <>
            ...
            <ResultBlock raw={resultMsg} roles={roles} outcome={storedOutcome} verdict={myRoleSlug === "boussole" || myRoleSlug === "chasseur_de_vampire"} />
          </>
        ) : (
          <div className="flex items-start gap-2">
...
export function CapabilityCard({
  tour, phase, created_at, payload, result,
  target_player_id, target_player_id_2,
  players, roles, highlight = false, kind = "capability",
}: {
  tour: number; phase: string; created_at: string;
  payload: Record<string, unknown>; result: Record<string, unknown> | null;
  target_player_id: string | null; target_player_id_2: string | null;
  players: { id: string; pseudo: string; role_meta?: unknown }[]; roles: Map<string, RoleRow>;
  highlight?: boolean;
  ...
  const itemIcon = itemSlug && (itemSlug in ITEM_CATALOG)
  ...
  const itemFac = isItem ? ((payload?.origin_faction as ItemOrigin | null | undefined) ?? null) : null;
  ...
  const tp2 = target_player_id_2 ? players.find((p) => p.id === target_player_id_2) ?? null : null;
  ...
  return (
    <div className="relative rounded-lg border border-border/60 bg-card/40 pl-3 pr-3 py-2.5">
      ...
      <div className="flex items-center gap-2">
        {isItem
          ? <span className="shrink-0 text-base leading-none" style={{ color: itemFacColor ?? undefined }}>{itemIcon}</span>
          : <Zap className={`shrink-0 size-4 ${accentText}`} aria-hidden />}
        <span className="text-[11px] font-semibold uppercase tracking-wider text-foreground/90">{headLabel}</span>
        {natureTag && (
              ...
              {targets.map((p) => {
                ...
                return (
                  <span key={p.id} className="inline-flex items-center gap-1 min-w-0">
                    <AvatarImg id={p.id} avatar={avatarOf(meta.avatar as string | undefined, p.id)} size={16} />
                    <span className="text-xs font-medium text-foreground/85 truncate">{p.pseudo}</span>
                  </span>
                );
              })}
            </div>
        ...
        <div className="flex-1" />
        <span className="text-[10px] text-muted-foreground tabular-nums shrink-0">T{tour} · {time}</span>
      </div>

      {/* Résultat */}
      <div className="mt-1.5">
        <ResultBlock raw={recap} roles={roles} size="sm" outcome={(result as Record<string, unknown> | null)?.outcome as ResultTone | undefined} verdict={(payload as Record<string, unknown> | null)?.effect === "compare"} />
      </div>
    </div>
  );
}

function HistoryTab({ gameId, meId, players, roles }: { gameId: string; meId: string; players: { id: string; pseudo: string; role_meta?: unknown }[]; roles: Map<string, RoleRow> }) {
  const [actions, setActions] = useState<ActionRow[]>([]);

  useEffect(() => {
    let cancelled = false;
    async function load() {
      const { data: a } = await supabase.from("role_actions").select("id,tour,phase,payload,result,created_at,target_player_id,target_player_id_2")
        .eq("game_id", gameId).eq("actor_player_id", meId).order("created_at", { ascending: false }).limit(60);
    ...
    void load();
    const ch = supabase.channel(`history-${meId}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "role_actions", filter: `actor_player_id=eq.${meId}` }, () => void load())
      .subscribe();
    return () => { cancelled = true; void supabase.removeChannel(ch); };
  }, [gameId, meId]);
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card/40 p-3 text-xs text-muted-foreground text-center">
        Retrouve ici <span className="text-foreground font-medium">tes capacités utilisées</span> et ce que tu as appris, tour par tour. <span className="text-muted-foreground/70">(Les objets sont dans l'onglet Inventaire.)</span>
      </div>
      ...
      {tours.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-card/20 p-8 text-center">
          ...
          <div className="text-xs text-muted-foreground mt-1">Tes actions et leurs résultats apparaîtront ici.</div>
        </div>
      ) : (
          ...
          {tours.map((t) => (
            <section key={t}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="text-xs font-semibold uppercase tracking-wider text-foreground">Tour {t}</div>
                <div className="flex-1 h-px bg-border" />
                <div className="text-[10px] text-muted-foreground">{byTour.get(t)!.length} action{byTour.get(t)!.length > 1 ? "s" : ""}</div>
              </div>
...
function WinTab({ me, myRole, players }: { me: import("@/engine/actions").PlayerRow; myRole: import("@/engine/actions").RoleRow | null; players: import("@/engine/actions").PlayerRow[] }) {
  const { factionLabel, lines } = describeWinCondition(me, myRole, players);
  const factionColor =
    myRole?.faction === "Civil" ? "text-sky-300 border-sky-500/40 bg-sky-500/5" :
    myRole?.faction === "Méchant" ? "text-destructive border-destructive/40 bg-destructive/5" :
    myRole?.slug === "vampire" ? "text-fuchsia-300 border-fuchsia-500/40 bg-fuchsia-500/5" :
  ...
  return (
    <div className="p-5 space-y-4">
      <div className={`rounded-xl border p-4 ${factionColor}`}>
        ...
        <div className="text-xs mt-1 opacity-80 inline-flex items-center gap-1"><RoleIcon role={myRole} size={16} /> {myRole?.name_fr}</div>
      </div>
      <div className="rounded-xl border border-border bg-card/40 p-4 space-y-2">
        <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Ta condition de victoire</div>
        {lines.map((l, i) => (
...
function ChatTab({ gameId, meId, mePseudo, isMedium, isDead }: { gameId: string; meId: string; mePseudo: string; isMedium: boolean; isDead: boolean }) {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from("role_actions")
        ...
        .order("created_at", { ascending: false })
        .limit(20);
      const row = (data ?? []).find((r) => (r as { payload: { effect?: string } }).payload?.effect === "temoin_reveal") as { target_player_id: string | null } | undefined;
      setTargetId(row?.target_player_id ?? null);
    }
    void load();
    const ch = supabase.channel(`temoin-${meId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "role_actions", filter: `actor_player_id=eq.${meId}` }, () => void load())
      .subscribe();
      ...
      {target && (
        <div className="mt-3 flex items-center gap-3 rounded-lg bg-background/40 border border-amber-500/30 p-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-amber-300/90 truncate">
              {targetRole?.name_fr ?? "?"}
            </div>
          </div>
          <div className="text-[10px] uppercase tracking-wider text-amber-400/80 font-mono">Civil</div>
        </div>
      )}
    </PanelCard>
  );
...
function NeighborWatchPanel({
  gameId, watchTargetId, watchSetCycle, players, roles, tour,
}: {
  gameId: string;
  ...
  useEffect(() => {
    if (!watchTargetId) { setActors([]); return; }
    async function load() {
      let q = supabase.from("role_actions")
    ...
    void load();
    const ch = supabase.channel(`voisin-${watchTargetId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "role_actions" }, () => void load())
      .subscribe();
    return () => { void supabase.removeChannel(ch); };
  }, [gameId, watchTargetId, watchSetCycle]);

  const targetName = watchTargetId ? players.find((p) => p.id === watchTargetId)?.pseudo ?? "?" : null;
  return (
    <PanelCard tone="indigo" icon={Radar} label="Surveillance">
      {!watchTargetId && <div className="mt-1 text-muted-foreground italic">Désigne quelqu'un à surveiller.</div>}
      {watchTargetId && (
        <>
          <div className="mt-1 text-foreground/90">
            Cible : <span className="font-semibold">{targetName}</span> · {actors.length} visiteur(s)
          </div>
          {actors.length > 0 && (
            <ul className="mt-1 space-y-0.5">
              {actors.map((a, i) => {
                ...
                const slug = (typeof pm.cover_slug === "string" ? pm.cover_slug : p?.role_slug) ?? "";
...
function AvocatPrisonPanel({
  ...
  const factionStyle = (faction: string | undefined): { bg: string; border: string; text: string; Icon: LucideIcon; label: string } => {
    switch (faction) {
      case "Civil":
        return { bg: "oklch(0.20 0.08 230 / 0.35)", border: "oklch(0.55 0.18 230 / 0.45)", text: "var(--citoyens)", Icon: Shield, label: "Civil" };
      case "Méchant":
        return { bg: "oklch(0.20 0.10 22 / 0.4)", border: "oklch(0.55 0.22 22 / 0.5)", text: "var(--mechants)", Icon: Swords, label: "Méchant" };
      case "Neutre":
        return { bg: "oklch(0.20 0.08 75 / 0.3)", border: "oklch(0.55 0.16 75 / 0.45)", text: "var(--neutres)", Icon: Orbit, label: "Neutre" };
      default:
        return { bg: "oklch(0.22 0.02 35 / 0.5)", border: "oklch(0.40 0.02 35 / 0.5)", text: "var(--muted-foreground)", Icon: CircleHelp, label: "Inconnu" };
    }
  };
  ...
  return (
    <PanelCard
      ...
      action={<span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-amber-500/18 text-amber-200">{prisoners.length}</span>}
    >
      ...
      {prisoners.length === 0 ? (
        ...
        >
          <span className="inline-flex items-center gap-1.5"><Bird className="size-4" aria-hidden /> Aucun joueur en prison actuellement.</span>
        </div>
      ) : (
          ...
          {prisoners.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-semibold truncate" style={{ color: "oklch(0.94 0.04 80)" }}>
                    {p.pseudo}
                  </div>
                  <div
...
function CartomancienBoardPanel({
  targetId, targetCycle, tour, players,
}: {
      ...
      <PanelCard tone="fuchsia" icon={Sparkles} label="Lecture des cartes">
        {!locked && (
          <div className="text-muted-foreground italic">
            Désigne 1 joueur ci-dessous. Une fois verrouillé, tu pourras consulter son tableau de suspicions. Le verrou se réinitialise à chaque nouveau tour.
          </div>
        )}
        {locked && target && (
          <div className="flex items-center justify-between gap-2">
            ...
            >
              <span className="inline-flex items-center gap-1.5"><Eye className="size-3.5" aria-hidden /> Voir le tableau</span>
            </button>
          </div>
        )}
      </PanelCard>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div>
                <div className="text-xs uppercase tracking-widest text-fuchsia-300 inline-flex items-center gap-1.5"><Sparkles className="size-3.5" aria-hidden /> Tableau live</div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {list.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`rounded-full ring-4 ring-offset-2 ring-offset-card ${RING_CLASS[lvl]}`}>
                        <AvatarImg avatar={playerAvatar(p)} size={56} className="bg-background" />
                      </div>
                      <div className="text-[11px] font-medium truncate w-full text-center">{p.pseudo}</div>
...
function VeuveNoireTrackerPanel({
  gameId, meId, pairs, players, tour,
}: {
  gameId: string;
  meId: string;
  ...
  useEffect(() => {
    if (targetIds.length === 0) { setVotes({}); return; }
    async function load() {
      ...
      for (const v of ((data ?? []) as Array<{ voter_player_id: string; target_player_id: string }>)) {
        map[v.voter_player_id] = v.target_player_id;
      }
      setVotes(map);
    }
    void load();
    const ch = supabase.channel(`veuve-${meId}-${tour}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "votes", filter: `game_id=eq.${gameId}` }, () => void load())
      .subscribe();
    return () => { void supabase.removeChannel(ch); };
  ...
  return (
    <PanelCard tone="purple" icon={Network} label={`Toile tendue · T${tour}`}>
      {!currentPair && (
        <div className="mt-1 text-muted-foreground italic">Aucune cible ce rassemblement — désigne 1 ou 2 joueurs.</div>
      )}
      {currentPair && (
        <ul className="mt-1 space-y-1">
          {targetIds.map((id) => {
            ...
            const voteTargetName = voteTargetId ? players.find((pp) => pp.id === voteTargetId)?.pseudo ?? "?" : null;
            const votedAgainstMe = voteTargetId === meId;
            return (
              <li key={id} className="flex items-center justify-between gap-2">
                <span className="font-semibold">{p?.pseudo ?? "?"}</span>
                <span className={votedAgainstMe ? "text-destructive font-bold" : "text-foreground/70"}>
                  {!voteTargetId && "n'a pas encore voté"}
                  {voteTargetId && votedAgainstMe && <span className="inline-flex items-center gap-1"><TriangleAlert className="size-3" aria-hidden /> a voté contre toi → mort au prochain rassemblement</span>}
...
function MiniDie({ value, size = 26 }: { value: number; size?: number }) {
  ...
  return (
    <svg viewBox="0 0 100 100" width={size} height={size} aria-label={`Dé ${value}`}>
      <rect x="6" y="6" width="88" height="88" rx="22" fill="oklch(0.97 0.02 90)" stroke="oklch(0.78 0.16 85)" strokeWidth="5" />
      {pips.map(([cx, cy], i) => (
...
function UsurpateurCoverChoice({
  ...
  async function pick(slug: string) {
    if (busy) return;
    setBusy(true);
    const { data: cur } = await supabase.from("players").select("role_meta").eq("id", meId).maybeSingle();
    const m = ((cur as { role_meta: Record<string, unknown> } | null)?.role_meta ?? {}) as Record<string, unknown>;
  ...
  return (
    <div className="mt-3 rounded-lg border border-violet-400/40 bg-violet-500/10 px-3 py-3 text-xs text-violet-200">
      <div className="font-semibold uppercase tracking-wider text-violet-300 mb-2 inline-flex items-center gap-1.5"><Drama className="size-3.5" aria-hidden /> Choisis ta couverture</div>
      <div className="grid grid-cols-3 gap-2">
        {choices.map((s) => {
          const r = roles.get(s);
          if (!r) return null;
          return (
            ...
            >
              ...
              <span className="text-[11px] font-semibold text-foreground text-center leading-tight">{r.name_fr}</span>
            </button>
          );
        })}
      </div>
      <div className="mt-2 italic opacity-80">Le choix est définitif. Toutes les enquêtes te désigneront ainsi.</div>
    </div>
  );
}
...
function VengeurBelovedChoice({
  ...
  const choices = proposedIds.length > 0
  ...
  async function pick(targetId: string) {
    ...
    const m = ((cur as { role_meta: Record<string, unknown> } | null)?.role_meta ?? {}) as Record<string, unknown>;
    await supabase.from("players").update({
      role_meta: { ...m, etre_cher: targetId, beloved_id: targetId, pending_beloved_choice: false } as never,
    }).eq("id", meId);
    setBusy(false);
  }
  ...
  return (
    <div className="mt-3 rounded-lg border border-pink-400/40 bg-pink-500/10 px-3 py-3 text-xs text-pink-200">
      <div className="font-semibold uppercase tracking-wider text-pink-300 mb-2 inline-flex items-center gap-1.5"><Heart className="size-3.5" aria-hidden /> Choisis ton être cher</div>
      {proposedIds.length > 0 && <div className="mb-2 opacity-80">Ces 2 joueurs sont des Civils. Choisis lequel sera ton être cher.</div>}
      {choices.length === 0 ? (
      ...
      <div className="mt-2 italic opacity-80">Le choix est définitif. Si il/elle meurt, tu recevras un couteau de vengeance.</div>
    </div>
  );
}
...
function GuetteurVisitsPanel({
  gameId, meId, players, roles, tour,
}: {
  gameId: string;
  meId: string;
  players: import("@/engine/actions").PlayerRow[];
  ...
  useEffect(() => {
    ...
    void load();
    const ch = supabase.channel(`guetteur-${meId}-${tour}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "role_actions", filter: `game_id=eq.${gameId}` }, () => void load())
      .subscribe();
    return () => { void supabase.removeChannel(ch); };
  }, [gameId, meId, tour]);
  return (
    <PanelCard tone="sky" icon={Eye} label={`Visites · Tour ${tour}`}>
      {actors.length === 0 && <div className="mt-1 text-muted-foreground italic">Personne ne t'a visité pour l'instant.</div>}
      {actors.length > 0 && (
        <ul className="mt-1 space-y-0.5">
          {actors.map((a, i) => {
            const p = players.find((pp) => pp.id === a.actor_player_id);
            return <li key={i} className="text-foreground/80">• {p?.pseudo ?? "?"}</li>;
          })}
        </ul>
      )}
    </PanelCard>
...
function JugePrisonPanel({
  players, currentTour, busy, blocked, remaining, total, onRelease,
}: {
  players: import("@/engine/actions").PlayerRow[];
  currentTour: number;
  busy: boolean;
  ...
  return (
    <PanelCard
      ...
      action={
        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${remaining <= 0 ? "bg-rose-500/20 text-rose-300 ring-1 ring-rose-400/40" : "bg-amber-500/15 text-amber-200 ring-1 ring-amber-400/30"}`}>
          {remaining <= 0 ? "Épuisé" : `${remaining}${total !== null ? ` / ${total}` : ""} restante${remaining > 1 ? "s" : ""}`}
        </span>
      }
    >
          ...
          {prisoners.map((p) => {
            ...
            return (
              <button
                ...
                title={isPending ? "Libération programmée pour le prochain tour" : ready ? "Sélectionner" : "Doit purger un tour complet"}
              >
                <span className="font-medium flex items-center gap-2"><AvatarImg avatar={playerAvatar(p)} size={24} /><Lock className="size-3" aria-hidden /> {p.pseudo}</span>
                {isPending && <span className="text-[9px] uppercase tracking-wider">libération programmée</span>}
                {!isPending && !ready && <span className="text-[9px] uppercase tracking-wider">vient d'arriver</span>}
              </button>
            );
          })}
        </div>
      ...
      >
        {busy ? "…" : <span className="inline-flex items-center gap-1.5"><LockOpen className="size-4" aria-hidden /> Programmer la libération</span>}
      </button>
    </PanelCard>
  );
}
...
function ExecuteurPrisonPanel({
  players, currentTour, busy, blocked, remaining, total, onExecute,
}: {
  players: import("@/engine/actions").PlayerRow[];
  currentTour: number;
  busy: boolean;
  ...
  const selectedReady = selected
    ? (() => {
        const p = prisoners.find((pp) => pp.id === selected);
        if (!p) return false;
        const since = ((p.role_meta as Record<string, unknown> | null)?.imprisoned_since_cycle as number | undefined) ?? currentTour;
  ...
  return (
    <PanelCard
      ...
      label={`Prisonniers — Tour ${currentTour}`}
      action={
        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${remaining <= 0 ? "bg-rose-500/30 text-rose-200 ring-1 ring-rose-400/60" : "bg-rose-500/15 text-rose-200 ring-1 ring-rose-400/30"}`}>
          {remaining <= 0 ? "Épuisé" : `${remaining}${total !== null ? ` / ${total}` : ""} restante${remaining > 1 ? "s" : ""}`}
        </span>
      }
    >
          ...
          {prisoners.map((p) => {
            ...
            return (
              <button
                key={p.id}
                disabled={!ready}
                onClick={() => { if (ready) { setSelected(isSel ? null : p.id); setConfirm(false); } }}
                className={`h-12 rounded-lg text-sm transition px-2 flex flex-col items-center justify-center ${
                  !ready
                    ? "bg-card/30 text-muted-foreground/40 cursor-not-allowed opacity-50"
                    : isSel
              ...
              >
                <span className="font-medium flex items-center gap-2"><AvatarImg avatar={playerAvatar(p)} size={24} /><Lock className="size-3" aria-hidden /> {p.pseudo}</span>
                {!ready && <span className="text-[9px] uppercase tracking-wider">vient d'arriver</span>}
              </button>
            );
          })}
        </div>
      ...
      >
        {busy ? "…" : confirm
          ? <span className="inline-flex items-center gap-1.5"><TriangleAlert className="size-4" aria-hidden /> Confirmer l'exécution</span>
          : <span className="inline-flex items-center gap-1.5"><Axe className="size-4" aria-hidden /> Exécuter</span>}
      </button>
    </PanelCard>
  );
}
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from("notifications")
        ...
        .eq("game_id", gameId).eq("player_id", meId).eq("type", "autopsy")
        .order("created_at", { ascending: false }).limit(10);
      setItems(((data ?? []) as Array<{ body: string; created_at: string }>));
    }
    void load();
    const ch = supabase.channel(`legiste-${meId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "notifications", filter: `player_id=eq.${meId}` }, () => void load())
      .subscribe();
    return () => { void supabase.removeChannel(ch); };
  }, [gameId, meId]);
  return (
    <PanelCard tone="rose" icon={Stethoscope} label="Autopsies récentes">
      {items.length === 0 && <div className="mt-1 text-muted-foreground italic">Aucune autopsie réalisée.</div>}
      {items.length > 0 && (
        <ul className="mt-1 space-y-0.5">
          {items.map((it, i) => (
            <li key={i} className="text-foreground/80">• {it.body}</li>
          ))}
        </ul>
      )}
    </PanelCard>
  ...
  if (used) {
    return (
      <PanelCard tone="neutral" icon={Target} label="Paranoïaque">
        <div className="text-muted-foreground">Capacité utilisée. Cible : <span className="font-semibold text-foreground">{pseudo}</span>.</div>
      </PanelCard>
    );
  }

  return (
    <PanelCard tone="amber" icon={Target} label="Ta cible" action={<span className="text-sm font-bold text-foreground">{pseudo}</span>}>
      <div className="text-[11px] text-muted-foreground mb-3">
        À toi de deviner sa faction. 1× dans la partie : protège-la ou tue-la (résolu au rassemblement).
      </div>
      <div className="grid grid-cols-2 gap-2">
        <button
      ...
      {targetDead && (
        <div className="mt-2 text-xs text-rose-300">Ta cible est morte — capacité inutilisable.</div>
      )}
      <button
