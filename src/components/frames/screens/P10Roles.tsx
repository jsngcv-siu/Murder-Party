...
export function P10Roles({ roles }: FrameContext) {
  ...
  const list = useMemo(() => {
    ...
    return arr.filter((r) => r.name_fr.toLowerCase().includes(k) || (r.description ?? "").toLowerCase().includes(k));
  }, [roles, q]);
    ...
    <div className="h-full flex flex-col bg-background p-5">
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      background: "radial-gradient(circle at 38% 30%, oklch(0.26 0.02 300), oklch(0.18 0.015 300))",
                  ...
                  <div className="font-display text-[11.5px] leading-tight mt-2 text-foreground line-clamp-2">{r.name_fr}</div>
                  <div className="text-[8px] uppercase tracking-wider mt-0.5" style={{ color: fc }}>{r.faction} · {r.type}</div>
                </button>
              );
            })}
          </div>
      ...
      {detail && (
        <div className="fixed inset-0 z-40 bg-background/95 backdrop-blur flex flex-col p-6 max-w-md mx-auto">
          <button onClick={() => setOpen(null)} className="self-start text-sm text-muted-foreground">← retour</button>
          <div className="mt-3 text-center">
            <div className="flex justify-center"><RoleIcon role={detail} size={96} /></div>
            <h2 className="mt-2 text-2xl font-bold text-gold" style={{ fontFamily: "var(--font-display)" }}>{detail.name_fr}</h2>
            ...
            <div className="mt-2 flex flex-wrap items-center justify-center gap-2">
              ...
              <span
                ...
                style={{
                  color: detail.faction === "Civil" ? "var(--citoyens)" : detail.faction === "Méchant" ? "var(--mechants)" : "var(--neutres)",
                  borderColor: detail.faction === "Civil" ? "oklch(0.72 0.16 230 / 0.35)" : detail.faction === "Méchant" ? "oklch(0.62 0.24 22 / 0.35)" : "oklch(0.80 0.12 300 / 0.40)",
                  backgroundColor: detail.faction === "Civil" ? "oklch(0.72 0.16 230 / 0.12)" : detail.faction === "Méchant" ? "oklch(0.62 0.24 22 / 0.12)" : "oklch(0.80 0.12 300 / 0.14)",
                }}
              >
                {detail.faction}
              </span>
              ...
              {(() => {
                ...
                const t = meta[detail.type ?? ""] ?? { icon: Shield, color: "var(--muted-foreground)" };
                const Icon = t.icon;
                return (
                  <span
              ...
              {/* Difficulté */}
              {(() => {
                const fcolor = detail.faction === "Civil" ? "var(--citoyens)" : detail.faction === "Méchant" ? "var(--mechants)" : "var(--neutres)";
                const bcolor = detail.faction === "Civil" ? "oklch(0.72 0.16 230 / 0.12)" : detail.faction === "Méchant" ? "oklch(0.62 0.24 22 / 0.12)" : "oklch(0.80 0.12 300 / 0.14)";
                const brcolor = detail.faction === "Civil" ? "oklch(0.72 0.16 230 / 0.35)" : detail.faction === "Méchant" ? "oklch(0.62 0.24 22 / 0.35)" : "oklch(0.80 0.12 300 / 0.40)";
                return (
                  <span
                    className="inline-flex items-center gap-1.5 rounded px-2 py-0.5 text-[10px] uppercase tracking-wider font-semibold border"
          ...
          <div className="mt-4 overflow-y-auto text-sm space-y-3">
            ...
            <div>
              ...
              <p className="whitespace-pre-wrap rounded bg-card border border-border p-3 text-sm">{detail.capacite_full_text}</p>
            </div>
            {(() => {
...
export function UsageCard({ role }: { role: { slug: string; usage_label: string; phase_activation: string | null } }) {
  ...
  return (
    <div className="mt-3 flex justify-center">
      <div className="inline-flex flex-col items-center gap-0.5 rounded-lg border border-gold/30 bg-gold/5 px-4 py-2 max-w-full">
        <div className="text-[9px] uppercase tracking-widest text-muted-foreground">Fréquence d'utilisation</div>
