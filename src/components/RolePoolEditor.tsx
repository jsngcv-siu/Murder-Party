...
export function RolePoolEditor({ game }: { game: GameRow }) {
  ...
  async function toggle(slug: string) {
    ...
    setBanned(next);
    setBusy(true);
    await supabase.from("games").update({ banned_roles: Array.from(next) as never }).eq("id", game.id);
    setBusy(false);
  }
  ...
  return (
    <div className="rounded-md border border-border bg-card/60">
      ...
      >
        <span className="flex items-center gap-2"><Ban className="size-4 text-destructive" /> Bannir des rôles ({banned.size})</span>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 max-h-80 overflow-y-auto">
          ...
          {Object.entries(groups).map(([faction, list]) => (
            <div key={faction}>
              <div className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">{faction}</div>
                  ...
                  return (
                    <button
                      ...
                      className={`text-left text-xs px-2 py-1.5 rounded border transition flex items-center gap-1.5 ${
                        locked ? "opacity-40 border-border bg-card" :
                        isBan ? "border-destructive/60 bg-destructive/10 text-destructive line-through" :
                        "border-border bg-card hover:bg-card/80"
                      }`}
                      title={locked ? "Rôle obligatoire" : isBan ? "Bannis — clique pour réactiver" : "Clique pour bannir"}
                    >
                      {isBan ? <Ban className="size-3 shrink-0" /> : <Check className="size-3 shrink-0 opacity-50" />}
                      <RoleIcon role={r} size={16} /><span className="truncate">{r.name_fr}</span>
                    </button>
                  );
                })}
              </div>
