...
import { Ban, Crosshair, Dices, Drama, Flag, FlaskConical, Hand, Heart, Lock, Martini, ShieldCheck, Skull, Sparkles, Wine, X, type LucideIcon } from "lucide-react";

type StatusRow = {
  id: string; status_slug: string; source: string | null;
  active_from_tour: number; active_until_tour: number | null;
...
const STATUS_BADGE: Record<string, { icon: LucideIcon; label: string; tone: Tone; desc: string }> = {
  protected:  { icon: ShieldCheck,  label: "Protégé",    tone: "sky",     desc: "Protection active : toute attaque te visant sera bloquée jusqu'à la fin de la durée." },
  poisoned:   { icon: FlaskConical, label: "Empoisonné", tone: "emerald", desc: "Poison en toi. Sans soin, c'est la mort au prochain rassemblement." },
  blocked:    { icon: Ban,          label: "Bloqué",     tone: "rose",    desc: "Ta capacité de rôle ET tes objets sont bloqués ce tour-ci. Tu ne peux rien utiliser." },
  blackmailed:{ icon: Hand,         label: "Chantage",   tone: "redDark", desc: "Tu es sous chantage : ta capacité et tes objets sont bloqués ce tour-ci." },
  drunk:      { icon: Wine,         label: "Ivre",       tone: "amber",   desc: "Le barman t'a servi un verre de trop : ta capacité et tes objets sont bloqués ce tour-ci." },
  blessed:    { icon: Sparkles,     label: "Béni",       tone: "yellow",  desc: "Bénédiction active. Une grâce te protège — vois les détails du rôle qui l'a accordée." },
  marked:     { icon: Flag,         label: "Suspect",    tone: "orange",  desc: "Un accusateur a jeté la suspicion sur toi en public. Cela peut influencer les votes." },
  manipulated:{ icon: Drama,        label: "Manipulé",   tone: "purple",  desc: "Le Marionnettiste te manipule : ta capacité a été détournée ou bloquée." },
  dice_loser: { icon: Dices,        label: "Perdant aux dés", tone: "redDark", desc: "Tu as perdu le pari du tricheur. À la prochaine annonce, tu seras mort — sauf si une protection te sauve d'ici là." },
  cible:      { icon: Crosshair,    label: "Ciblé",       tone: "redDark", desc: "Le Tueur Stratège t'a marqué. Tu mourras à la prochaine annonce — sauf si une protection te sauve, ou s'il est neutralisé d'ici là." },
};

export function StatusBandeau({ me, tour, players }: { me: PlayerRow; tour: number; players?: PlayerRow[] }) {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase
        ...
        .eq("player_id", me.id);
      const rows = ((data ?? []) as StatusRow[]).filter((s) =>
        s.active_from_tour <= tour && (s.active_until_tour == null || s.active_until_tour >= tour)
      );
      setStatuses(rows);
    };
    void load();
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "player_statuses", filter: `player_id=eq.${me.id}` }, () => void load())
      .subscribe();
  ...
  const badges: Badge[] = [];
  const pushDef = (key: string, def: { icon: LucideIcon; label: string; tone: Tone; desc?: string } | undefined) => {
    if (!def) return;
  ...
  if (blackmail && blackmail >= tour && blackmailFrom <= tour) pushDef("blackmailed", STATUS_BADGE.blackmailed);
  ...
  const linkedId = (meta.linked_with as string | undefined) ?? (meta.linked_to as string | undefined);
  if (linkedId) {
    const partner = players?.find((p) => p.id === linkedId);
    badges.push({ key: "lover", icon: Heart, label: partner ? `Amoureux · ${partner.pseudo}` : "Amoureux",
      tone: "pink",
  ...
  if (strategeMark && (strategeMark.resolves_tour ?? -1) >= tour) pushDef("cible", STATUS_BADGE.cible);
  ...
  for (const s of statuses) {
    ...
    if (s.status_slug === "good_time") {
      const partner = (s.payload?.partner_pseudo as string | undefined)
  ...
  // Mort / prison
  if (!me.is_alive) badges.unshift({ key: "dead", icon: Skull, label: "Mort", tone: "deadGreen",
    desc: "C'est la fin pour toi. Tu peux observer la partie et le chat des morts." });
  else if (me.is_imprisoned) badges.unshift({ key: "prison", icon: Lock, label: "Prison", tone: "stone",
