...
export function describeWinCondition(
  ...
  if (role.faction === "Civil") {
    base.push("🟦 Tu gagnes si tous les Méchants, Vampires, Amoureux et Neutres adverses sont éliminés ou emprisonnés.");
  ...
  switch (role.slug) {
    case "veuve_noire": {
      base.length = 0;
      base.push("🕷️ Tu gagnes en SOLO si tu es la seule personne encore en vie et libre du manoir.");
      break;
    }
    case "parieur_tricheur": {
      base.length = 0;
      base.push("🎲 Tu gagnes en SOLO si tu es la seule personne encore en vie et libre, sans avoir perdu au dé.");
      break;
    }
    case "chasseur_de_vampire": {
      base.length = 0;
      base.push("🧛 Apparu·e après la 1re morsure du Vampire, tu traques les Vampires : désigne un joueur pour savoir s'il en est un (et l'éliminer si oui).");
      base.push("🛡️ Tu es immunisé·e contre la morsure et ne peux pas être converti·e.");
      base.push("🤝 Allié·e des Civils : tu gagnes avec eux quand tous les Méchants, Vampires et Neutres adverses sont écartés.");
      break;
    }
    case "empoisonneur": {
      const others = allPlayers.filter((p) => !p.is_mj && p.id !== me.id && p.is_alive && !p.is_imprisoned);
      const poisoned = others.filter((p) => ((p.role_meta ?? {}) as Record<string, unknown>).poisoned === true).length;
      base.length = 0;
      base.push(`☠️ Tu gagnes quand tous les survivants libres sont empoisonnés (${poisoned}/${others.length}).`);
      break;
    }

    case "heritier_dechu": {
      base.length = 0;
      base.push("👑 Tu gagnes en vie avec les Méchants à la fin de la partie (tant qu'un Méchant survit).");
      break;
    }
    case "oracle": {
      ...
      base.length = 0;
      if (proph) {
        base.push(`🔮 Prophétie verrouillée : victoire des ${proph}. Tu gagnes si cette faction l'emporte et que tu es en vie à la fin.`);
      } else {
        base.push("🔮 À la 1ère phase libre, prédis quelle faction (Civils, Méchants ou Neutres) gagnera. Tu gagnes avec elle si tu es en vie à la fin.");
      }
      break;
    }
    case "imitateur": {
      ...
      if (imitated) {
        ...
        base.push("🎭 Une fois dans la partie, copie le rôle du dernier mort : tu hériteras alors de sa faction et de sa condition de victoire.");
      }
      break;
    }
    case "vengeur": {
      const cher = findName(m.beloved_id);
      if (cher) base.push(`💔 Tant que ${cher} (ton être cher) vit, ta capacité de tuer reste verrouillée.`);
      break;
    }
    case "entremetteur": {
      const pair = (m.linked_pair as string[] | undefined) ?? [];
      const names = pair.map((id) => allPlayers.find((p) => p.id === id)?.pseudo ?? "?").join(" & ");
      base.length = 0;
      base.push(`💞 Victoire DURE : le couple lié (${names || "—"}) et toi êtes les seuls survivants.`);
      base.push("🩹 Repli : si le couple meurt, tu gagnes quand même en survivant en vie et libre jusqu'à la fin, quel que soit le camp vainqueur.");
      break;
    }
    case "conservateur": {
      base.length = 0;
      base.push("🗝️ À chaque phase libre et à chaque rassemblement, tu confies une relique maudite au hasard à une cible.");
      base.push("🫀 Tu gagnes (victoire SOLO immédiate) dès qu'une personne reçoit la relique « Le Cœur du Manoir ».");
      break;
    }
  }
