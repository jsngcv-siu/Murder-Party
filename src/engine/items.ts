...
export const ITEM_CATALOG: Record<ItemSlug, Omit<Item, "id" | "received_at" | "received_from" | "payload">> = {
  fiole_mort: {
    slug: "fiole_mort",
    name: "Fiole de mort",
    icon: "☠️",
    description: "Donne la mort à une cible : elle mourra au prochain rassemblement, sans laisser de trace de poison.",
  },
  fiole_vie: {
    slug: "fiole_vie",
    name: "Fiole de vie",
...
export function buildItem(slug: ItemSlug, opts: { from?: string; payload?: Record<string, unknown>; nameOverride?: string; descriptionOverride?: string; iconOverride?: string; originFaction?: ItemOrigin } = {}): Item {
...
export function itemNeedsTarget(slug: ItemSlug, payload?: Record<string, unknown> | null): "single" | "none" {
...
type ReliqueDef = { name: string; icon: string; description: string; weight: number; effect?: ReliqueEffect };

export const RELIQUE_CATALOG: Record<ReliqueVariant, ReliqueDef> = {
  coeur_du_manoir:        { name: "Le Cœur du Manoir",            icon: "🫀", weight: 5.0,  effect: "special_win",
    description: "Relique ultime — si tu la révèles, le Conservateur remporte la partie. Tout le monde perd." },
  oeil_damnation:         { name: "L'Œil de la Damnation",        icon: "👁️", weight: 4.0,  effect: "reveal_random",
    description: "Révèle le rôle d'un joueur au hasard." },
  medaillon_vieux_maitre: { name: "Le Médaillon du Vieux Maître", icon: "🏅", weight: 4.0,  effect: "protect_self",
    description: "Te protège pendant 1 jour entier." },
  lettre_scellee:         { name: "La Lettre Scellée",            icon: "✉️", weight: 4.0,  effect: "block_target",
    description: "Bloque la capacité d'un joueur ciblé pendant 1 jour." },
  miroir_minuit:          { name: "Le Miroir de Minuit",          icon: "🪞", weight: 10.0,
    description: "Un reflet glacé, sans pouvoir. Pure beauté maudite." },
  clef_aile_interdite:    { name: "La Clé de l'Aile Interdite",   icon: "🗝️", weight: 12.0,
    description: "Elle n'ouvre plus rien — ou alors plus rien d'utile." },
  poupee_grenier:         { name: "La poupée du grenier",         icon: "🪆", weight: 14.0,
    description: "Elle te regarde fixement. Aucune capacité." },
  lettre_oubliee:         { name: "La Lettre Oubliée",            icon: "📜", weight: 16.0,
    description: "Une lettre jamais lue, jamais envoyée. Aucun effet." },
  portrait_dame_blanche:  { name: "Le Portrait de la Dame Blanche", icon: "🖼️", weight: 17.5,
    description: "Son regard te suit. C'est tout." },
  bougie_des_ames:        { name: "La Bougie des Âmes",           icon: "🕯️", weight: 13.5,
    description: "Sa flamme vacille — sans réelle utilité." },
};

const RELIQUES_WITH_EFFECT = new Set<string>([
  "coeur_du_manoir", "oeil_damnation", "medaillon_vieux_maitre", "lettre_scellee",
]);
...
export async function useItem(opts: {
  gameId: string;
  actorId: string;
  actorPseudo: string;
  item: Item;
  ...
  if (!itemIsUsable(item.slug, item.payload)) return { ok: false, message: "Cet objet n'a pas d'effet actif." };
  if (item.consumed) return { ok: false, message: "Objet déjà utilisé." };
  if (itemNeedsTarget(item.slug, item.payload) === "single" && !target) return { ok: false, message: "Cible requise" };
  ...
  if (isApoOwnFiole && (((actorMeta.fioles_self_used as number | undefined) ?? 0) >= 1)) {
    return { ok: false, message: "Tu ne peux garder qu'une seule fiole pour toi — les autres doivent être offertes." };
  }
  ...
  switch (item.slug) {
    case "lettre": {
      if (!target) { message = "Cible requise pour la lettre anonyme."; break; }
      const raw = String(item.payload?.message ?? "").trim();
      if (!raw) { return { ok: false, message: "Écris un message avant d'envoyer." }; }
      ...
      await notify(target.id, "📨 Nouvelle lettre", `Tu as reçu une lettre de ${opts.actorPseudo}.`);
    ...
    case "fiole_vie": {
      ...
      await submitIntent({
        gameId, tour, phase: "free",
        actorId, targetId: target!.id,
        category: "CURE", timing: "DEFERRED",
        source: "item:fiole_vie",
        payload: { target_pseudo: target!.pseudo },
      });
      await notify(target!.id, "💚 Soigné", "Une fiole de vie te protège pour le prochain rassemblement.");
    ...
    case "couteau": {
      const { submitIntent } = await import("./resolver");
      const mechantOrigin = (item.payload as Record<string, unknown> | null)?.mechant_origin === true;
      ...
      switch (item.received_from) {
        case "Cuisine": weaponFromSlug = "cuisinier"; break;
        case "Vengeance": weaponFromSlug = "vengeur"; break;
        case "Stratège": weaponFromSlug = "stratege"; break;
        case "Inconnu": weaponFromSlug = "armurier"; break;
        default: weaponFromSlug = null;
      }
      await submitIntent({
        gameId, tour, phase: "free",
        actorId, targetId: target!.id,
      ...
      const giftedById = (item.payload as Record<string, unknown> | null)?.gifted_by_id as string | undefined;
      if (giftedById) {
        await notify(giftedById, "🔪 Ton couteau a frappé", `${opts.actorPseudo} a utilisé couteau sur ${target!.pseudo}.`);
      }
    ...
    case "relique": {
      ...
      if (variant === "coeur_du_manoir") {
        ...
        await endGameWithWinner(gameId, "Conservateur", `${opts.actorPseudo} a révélé Le Cœur du Manoir. Le Manoir le reconnaît comme son gardien.`);
        message = "🫀 Le Cœur du Manoir bat dans tes mains — toutes les factions s'inclinent. Victoire du Conservateur.";
      } else if (variant === "oeil_damnation") {
        // Révèle le rôle d'un joueur vivant au hasard (≠ porteur)
        const { data: pool } = await supabase
          .from("players").select("id, pseudo, role_slug")
          .eq("game_id", gameId).eq("is_alive", true).eq("is_mj", false)
          .neq("id", actorId);
        const list = (pool ?? []) as Array<{ id: string; pseudo: string; role_slug: string | null }>;
        if (list.length === 0) {
        ...
        await supabase.from("players").update({ role_meta: { ...meta0, protected_until_cycle: protUntil } as never }).eq("id", actorId);
        ...
        if (!target) { message = "Cible requise pour La Lettre Scellée."; break; }
        const { data: row } = await supabase.from("players").select("role_meta").eq("id", target.id).maybeSingle();
        const meta0 = ((row?.role_meta ?? {}) as Record<string, unknown>);
        const blockUntil = Math.max((meta0.blocked_until_cycle as number | undefined) ?? -1, tour);
        await supabase.from("players").update({ role_meta: { ...meta0, blocked_until_cycle: blockUntil } as never }).eq("id", target.id);
        message = `✉️ La Lettre Scellée bloque la capacité de ${target.pseudo} pour le jour.`;
        await notify(target.id, "✉️ Capacité scellée", "Une Lettre Scellée bloque ta capacité pour le restant du jour.");
  ...
  const { data: row } = await supabase.from("players").select("role_meta").eq("id", actorId).maybeSingle();
  const meta = ((row?.role_meta ?? {}) as Record<string, unknown>);
  const inv = ((meta.inventory as Item[] | undefined) ?? []).map((it) => it.id === item.id ? { ...it, consumed: true, payload: item.payload ?? it.payload } : it);
  ...
  await supabase.from("players").update({ role_meta: { ...meta, inventory: inv, last_item_use_cycle: tour, ...apoPatch } as never }).eq("id", actorId);

  // Trace dans role_actions pour l'historique du joueur
  await supabase.from("role_actions").insert({
    game_id: gameId, actor_player_id: actorId, tour, phase: "free",
    ...
    payload: { item: item.slug, name: item.name, origin_faction: itemFaction(item), object_capacity: item.received_from === "Cuisine" } as never,
