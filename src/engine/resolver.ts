...
export type IntentCategory =
  | "ATTACK" | "PROTECT" | "CURE" | "INVESTIGATE"
  | "BLOCK" | "FALSIFY" | "CASCADE" | "TRANSFER" | "CONVERT" | "META";
...
const LAYER_BY_CATEGORY: Record<IntentCategory, number> = {
  ...
  INVESTIGATE: 99, BLOCK: 99, FALSIFY: 99,
  TRANSFER: 99, META: 99,
};
...
async function setStatus(opts: {
  gameId: string; playerId: string; statusSlug: string; source: string;
  fromTour: number; untilTour?: number | null; payload?: Record<string, unknown>;
}) {
...
async function decrementCharge(itemId: string | null | undefined): Promise<void> {
  ...
  if (next === 0) {
    ...
    await supabase.from("inventory").update({ charges: next, updated_at: new Date().toISOString() }).eq("id", itemId);
  }
}
...
function deferredPlayerResult(
  ...
  if (cat === "ATTACK") {
    ...
    if (status === "applied") return { summary: "Attaque réussie — ta cible n'a pas survécu.", outcome: "success" };
    if (status === "protected") return { summary: "Échec : ta cible a survécu à ton attaque.", outcome: "fail" };
    return { summary: "Ton attaque n'a pas pu aboutir ce tour.", outcome: "fail" };
  }
  if (cat === "PROTECT") {
    if (status === "applied") return { summary: "Protection posée sur ta cible ce tour.", outcome: "success" };
  ...
  if (cat === "CASCADE") {
    ...
    if (status === "protected") return { summary: "Échec : ta cible était protégée.", outcome: "fail" };
...
async function applyResultToLatestAction(
  gameId: string, actorId: string, tour: number,
  result: Record<string, unknown>,
): Promise<void> {
  const { data: latest } = await supabase.from("role_actions")
    .select("id")
    .eq("game_id", gameId).eq("actor_player_id", actorId).eq("tour", tour)
    .order("created_at", { ascending: false }).limit(1).maybeSingle();
  const id = (latest as { id: string } | null)?.id;
  if (id) await supabase.from("role_actions").update({ result: result as never }).eq("id", id);
}
...
async function checkPreconditions(
  ...
  const m = (actor.role_meta ?? {}) as Record<string, unknown>;
  const isRoleSource = (intent.source ?? "").startsWith("role:");
  const isItemSource = (intent.source ?? "").startsWith("item:");
  if (isRoleSource) {
    const blockedUntil = (m.blocked_until_cycle as number | undefined) ?? -1;
    const blockedFrom = (m.blocked_from_cycle as number | undefined) ?? -Infinity;
    const blackmailUntil = (m.blackmail_until_cycle as number | undefined) ?? -1;
    const blackmailFrom = (m.blackmail_from_cycle as number | undefined) ?? -Infinity;
    if (blockedUntil >= intent.tour && blockedFrom <= intent.tour) return { ok: false, reason: "blocked" };
    if (blackmailUntil >= intent.tour && blackmailFrom <= intent.tour) return { ok: false, reason: "blackmailed" };
  }
  ...
  if (isItemSource && intent.item_id) {
    ...
    if (inv.holder_player_id !== intent.actor_player_id) return { ok: false, reason: "item_transferred" };
...
async function applyProtect(
  ...
  const m = ((tgt as { role_meta: Record<string, unknown> } | null)?.role_meta ?? {});
  const nextMeta: Record<string, unknown> = { ...m, protected_until_cycle: shieldUntil };
...
async function applyCure(
  ...
  await setStatus({
    gameId: intent.game_id, playerId: targetId, statusSlug: "protected",
    source: intent.source ?? "unknown", fromTour: intent.tour, untilTour: shieldUntil,
  });
  await decrementCharge(intent.item_id);
...
async function applyAttack(
  intent: RoleActionRow,
  killer: (gameId: string, playerId: string, reason: string, attackerId?: string, extra?: Record<string, unknown>) => Promise<boolean>,
): Promise<Record<string, unknown>> {
  const targetId = intent.target_player_id;
  if (!targetId) return { status: "cancelled", reason: "no_target" };
  ...
  const prot = (tRow.role_meta?.protected_until_cycle as number | undefined) ?? -1;
  const blessedUntil = (tRow.role_meta?.blessed_until_cycle as number | undefined) ?? -1;
  const isBlessed = (tRow.role_meta?.blessed_by_saint === true) && blessedUntil >= intent.tour;
  if (prot >= intent.tour || isBlessed) {
    ...
    await notifyMJ({
      gameId: intent.game_id, type: "shielded",
    ...
    const saintId = tRow.role_meta?.blessed_by_saint_id as string | undefined;
    if (isBlessed && saintId) {
      ...
      const targetPseudo = (intent.payload as Record<string, unknown> | null)?.target_pseudo ?? "ta cible";
      await notify({
        gameId: intent.game_id, playerId: saintId, type: "saint_block_log",
        title: "✨ Bénédiction active",
        body: `${attackerPseudo} a tenté une attaque (${intent.source ?? "?"}) sur ${targetPseudo} — annulée.`,
        payload: {
          actor_id: intent.actor_player_id, actor_pseudo: attackerPseudo,
          target_id: targetId, target_pseudo: targetPseudo,
          action: intent.source ?? "attaque", tour: intent.tour,
        },
      });
      await notify({
        gameId: intent.game_id, playerId: intent.actor_player_id, type: "saint_block",
        title: "✨ Cible bénie",
    ...
    const guardedBy = tRow.role_meta?.guarded_by as string | undefined;
    const payloadAttack = (intent.payload as Record<string, unknown> | null) ?? {};
    const isMechantAttack = intent.source === "role:tueur"
    ...
    if (isMechantAttack && typeof guardedBy === "string" && guardedBy.length > 0) {
      ...
      return { status: "protected", reason: "majordome_trade", guard: guardedBy, killer: intent.actor_player_id };
    }
  ...
  if (subEffect === "poison_delayed") {
    ...
    await decrementCharge(intent.item_id);
    return { status: "applied", effect: "poison_delayed", target: targetId, resolves_at: resolvesAt };
  }
  // Kill direct (Tueur, fiole_mort, marionnette forcée).
  const payload = (intent.payload as Record<string, unknown> | null) ?? {};
  const reason = payload.kill_reason as string | undefined
    ?? (intent.source?.startsWith("item:") ? intent.source.replace("item:", "") : intent.source?.replace("role:", "") ?? "engine");
  ...
  if (payload.puppet === true) {
    ...
    await notify({
      gameId: intent.game_id, playerId: intent.actor_player_id, type: "puppet_forced",
    ...
    if (puppeteerId) {
      await notify({
        gameId: intent.game_id, playerId: puppeteerId, type: "puppet_mirror",
  ...
  const ok = await killer(intent.game_id, targetId, reason, intent.actor_player_id, weaponFromSlug ? { weapon_from_slug: weaponFromSlug } : undefined);
  await decrementCharge(intent.item_id);
...
async function applyConvert(
  intent: RoleActionRow,
  converter: (gameId: string, vampireId: string, targetId: string, tour: number) => Promise<boolean>,
): Promise<Record<string, unknown>> {
  const targetId = intent.target_player_id;
  if (!targetId) return { status: "cancelled", reason: "no_target" };
  const { data: actorRow } = await supabase
    .from("players").select("is_alive").eq("id", intent.actor_player_id).single();
  if (!(actorRow as { is_alive: boolean } | null)?.is_alive) {
    return { status: "cancelled", reason: "biter_killed" };
  }
  const { data: tgtRow } = await supabase
    .from("players").select("is_alive").eq("id", targetId).single();
...
async function applyPoison(
  ...
  const cur = (tRow.role_meta ?? {});
  await supabase.from("players").update({
    role_meta: { ...cur, poisoned: true, poisoned_by: intent.actor_player_id, poisoned_at_cycle: intent.tour } as never,
  }).eq("id", targetId);
  await setStatus({
    gameId: intent.game_id, playerId: targetId, statusSlug: "poisoned",
    source: intent.source ?? "unknown", fromTour: intent.tour, untilTour: null,
  });
...
async function preprocessCleaner(
  ...
  const { data: cleanersRows } = await supabase
    .from("players").select("*")
    .eq("game_id", gameId).eq("role_slug", "cleaner").eq("is_alive", true);
  const cleaners = (cleanersRows ?? []) as PlayerRow[];
  const armed = cleaners.find(
    (c) => (c.role_meta as Record<string, unknown> | null)?.clean_armed === true,
  );
  if (!armed) return;
  ...
  const { count } = await supabase
    ...
    .eq("game_id", gameId).eq("is_mj", false);
  const maxUses = (count ?? 0) >= 10 ? 2 : 1;
  const cleanerMeta = (armed.role_meta ?? {}) as Record<string, unknown>;
  const usesMap = { ...((cleanerMeta.uses ?? {}) as Record<string, number>) };
  ...
  for (const it of picked) {
    ...
    const tMeta = (tgt.role_meta ?? {}) as Record<string, unknown>;
    await supabase.from("players").update({
      role_meta: { ...tMeta, cleaned: true } as never,
    }).eq("id", tid);
    await notify({
      gameId, playerId: armed.id, type: "clean_done",
      title: "🧹 Corps nettoyé",
...
export async function resolveDeferredIntents(
  ...
  for (const intent of intents) {
    ...
    if (cat === "ATTACK" && intent.source === "role:parieur_tricheur" && intent.target_player_id) {
      await supabase.from("player_statuses").delete()
        .eq("game_id", gameId)
    ...
    if (!pre.ok) {
      ...
      if (cat === "ATTACK") await decrementCharge(intent.item_id);
      {
        ...
        await writeResolution(intent.id, { status: "cancelled", reason: pre.reason }, pr ?? undefined);
    ...
    else { res = { status: "cancelled", reason: "unsupported_category" }; }
