...
const meta = (p: { role_meta: unknown } | null | undefined): Meta =>
  ((p?.role_meta ?? {}) as Meta);
...
async function patchMeta(playerId: string, patch: Meta) {
  ...
  await supabase.from("players").update({ role_meta: next as never }).eq("id", playerId);
  return next;
}

// ─────────────── Bénédiction du Saint ───────────────
/** Faction "hostile" pour les checks de bénédiction : Méchants + Neutres MAL/CHAOS. */
function isHostileFactionType(faction: string | null | undefined, type: string | null | undefined): boolean {
...
export async function tryBlessingBlock(opts: {
  ...
  actorRole: { faction: string | null; secondary_type?: string | null } | { faction: string | null; type?: string | null };
  ...
  if (saintId) {
    await notify({
      gameId: opts.gameId, playerId: saintId, type: "saint_block_log",
      title: "✨ Bénédiction active",
      body: `${opts.actor.pseudo} a tenté « ${opts.actionLabel} » sur ${opts.target.pseudo} — action annulée.`,
      payload: {
        actor_id: opts.actor.id, actor_pseudo: opts.actor.pseudo,
        target_id: opts.target.id, target_pseudo: opts.target.pseudo,
        action: opts.actionLabel, tour: opts.tour, phase: opts.phase,
      },
    });
  }
  await notify({
    gameId: opts.gameId, playerId: opts.actor.id, type: "saint_block",
    title: "✨ Cible bénite",
...
export function allowedActivePhases(role: RoleRow): Set<Phase> {
  ...
  const src = `${role.usage_label ?? ""} ${(role as { frequency_label?: string | null }).frequency_label ?? ""} ${role.phase_activation ?? ""}`.toLowerCase();
...
export function whyCannotUse(role: RoleRow, m: Meta, tour: number, playerCount: number, phase?: Phase): string | null {
  if (((m.blackmail_until_cycle as number | undefined) ?? -1) >= tour && ((m.blackmail_from_cycle as number | undefined) ?? -Infinity) <= tour) return "Sous chantage";
  if (((m.blocked_until_cycle as number | undefined) ?? -1) >= tour && ((m.blocked_from_cycle as number | undefined) ?? -Infinity) <= tour) return "Capacité bloquée";
  if (phase) {
  ...
  if (isPerCycle(role)) {
    if (role.slug === "conservateur") {
      ...
      if (usedNow >= limit) return limit > 1 ? `Déjà utilisé ${limit}× ce tour` : "Déjà utilisé ce tour";
    }
  }
  return null;
}
...
async function markUsage(actor: PlayerRow, role: RoleRow, tour: number, phase?: Phase) {
  ...
  const used_cycle_count = { ...((m.used_cycle_count ?? {}) as Record<string, { tour: number; count: number }>) };
  const prev = used_cycle_count[role.slug];
  used_cycle_count[role.slug] = prev && prev.tour === tour
    ? { tour, count: prev.count + 1 }
    : { tour, count: 1 };
...
function drawByQuotas(
  ...
  for (const [type, q] of Object.entries(quotas)) {
    ...
    const need = Math.min(Math.max(0, q.min - (countByType[type] ?? 0)), bucket.length, totalSlots - picked.length);
    if (need <= 0) continue;
    const drawn = weightedDraw(bucket, need);
    picked.push(...drawn);
    countByType[type] = (countByType[type] ?? 0) + drawn.length;
    for (const r of drawn) {
      const i = bucket.indexOf(r); if (i >= 0) bucket.splice(i, 1);
    }
  }
  ...
  while (picked.length < totalSlots) {
    ...
    if (bucket) { const i = bucket.indexOf(r); if (i >= 0) bucket.splice(i, 1); }
  }
...
export async function drawRoles(
  ...
  const civilPool = eligible.filter(
    (r) => r.faction === "Civil" && !slugs.includes(r.slug),
  );
  ...
  for (const s of slugs) {
    ...
    if (r?.faction === "Civil" && r.type) baseCivilTypes[r.type] = (baseCivilTypes[r.type] ?? 0) + 1;
  }
  const civilPicked = drawByQuotas(civilPool, civilQuotasFor(playerCount), remaining, baseCivilTypes);
...
export async function rollRoles(gameId: string): Promise<string[]> {
  ...
  const drawablePlayers = game.mode_detective_player
    ? players
    : players.filter((p) => !p.is_mj);
  ...
  if (unassigned.length > 0) {
    ...
    if (cfg && cfg.slots.length > 0) {
      ...
      if (picked.length < unassigned.length) {
        const extra = await drawRoles(unassigned.length - picked.length, game.mode_detective_player, [...allBanned, ...picked]);
        picked.push(...extra);
      }
    ...
    for (let i = 0; i < unassigned.length; i++) {
      await supabase.from("players")
        .update({ role_slug: drawnSlugs[i], is_alive: true, is_imprisoned: false, role_meta: keepCosmeticMeta(unassigned[i].role_meta) as never })
  ...
  for (const p of manuallyAssigned) {
    await supabase.from("players")
      .update({ is_alive: true, is_imprisoned: false, role_meta: keepCosmeticMeta(p.role_meta) as never })
      .eq("id", p.id);
  }
  emit("roll", `🎲 Rôles tirés — ${drawablePlayers.length} joueurs (${manuallyAssigned.length} manuels)`, { gameId, slugs: drawnSlugs });
  return drawnSlugs;
}
...
async function phaseDurationFor(gameId: string, phase: Phase): Promise<number> {
  ...
  const { data } = await supabase
    ...
    .eq("id", gameId)
    .maybeSingle();
  const row = (data ?? {}) as { phase_duration_free_s: number | null; phase_duration_gathering_s: number | null; phase_duration_vote_s: number | null };
...
async function applySetupEffects(gameId: string) {
  ...
  // (recollées à la table, IRL — aucune fusion codée).
  {
    ...
    for (const g of grants) {
      await grantItem(g.playerId, buildItem("indice", {
        from: "Manoir",
      ...
      await notify({
        gameId, playerId: g.playerId, type: "indice_setup",
        title: "🧩 Indice reçu",
        body: "Tu as reçu un indice. Consulte-le dans ton inventaire.",
        mjTitle: "🧩 Indice", mjBody: `${who} a reçu un indice${g.fragment ? " (fragment)" : ""}.`,
      });
  ...
  if (temoin) {
    ...
    if (pick) {
      ...
      await notify({
        gameId, playerId: temoin.id, type: "temoin_reveal",
        title: "👁️ Témoin", body,
        mjTitle: "👁️ Témoin",
  ...
  if (entremetteur) {
    ...
    await notify({
      gameId, playerId: entremetteur.id, type: "entremetteur_setup",
    ...
    await logSetup(entremetteur.id, "Tu choisiras 2 joueurs à lier à la 1ère phase libre.", { effect: "entremetteur_pending" });
  }
  ...
  if (vengeur) {
    ...
    const choices = shuffle(civils).slice(0, 2).map((p) => p.id);
    const choiceNames = choices.map((id) => alive.find((p) => p.id === id)?.pseudo ?? "?").join(" · ");
    await patchMeta(vengeur.id, { pending_beloved_choice: true, etre_cher: null, beloved_id: null, vengeur_choices: choices });
    await notify({
      gameId, playerId: vengeur.id, type: "vengeur_setup",
      ...
      mjTitle: "🤍 Vengeur", mjBody: `${vengeur.pseudo} (Vengeur) choisira son être cher parmi : ${choiceNames || "(aucun civil)"}.`,
    });
    await logSetup(vengeur.id, `Tu choisiras ton être cher${choiceNames ? ` parmi 2 Civils : ${choiceNames}` : ""} à la 1ère phase libre.`, { effect: "vengeur_pending" });
  }
  ...
  if (usurpateur) {
    const absent = shuffle(Array.from(rolesBySlug.values())
      .filter((r) => !allSlugs.has(r.slug) && r.faction === "Civil" && !r.emergent));
    ...
    if (choices.length > 0) {
      ...
      await notify({ gameId, playerId: usurpateur.id, type: "cover_pending", title: "🎭 Choisis ta couverture", body: `À la prochaine phase libre : ${labels}`, mjTitle: "🎭 Usurpateur", mjBody: `${usurpateur.pseudo} (Usurpateur) doit choisir parmi : ${labels}.` });
      await logSetup(usurpateur.id, `À la prochaine phase libre, choisis ta couverture parmi : ${labels}.`, { effect: "cover_pending" });
    }
  }
  ...
  if (ange) {
    ...
    if (tgt) {
      ...
      await notify({ gameId, playerId: ange.id, type: "ward", title: "🛡️ Ta cible", body: `Tu veilles sur ${tgt.pseudo}.`, mjTitle: "🛡️ Ange Gardien", mjBody: `${ange.pseudo} (Ange Gardien) surveille ${tgt.pseudo} (${r?.icon} ${r?.name_fr}).` });
  ...
  if (parano) {
    ...
    if (tgt) {
      await patchMeta(parano.id, { paranoid_target_id: tgt.id, paranoid_target_pseudo: tgt.pseudo });
      const body = `Ta cible : ${tgt.pseudo}. À toi de deviner s'il est de ton côté. 1× dans la partie : protège-le ou tue-le.`;
      await notify({
        gameId, playerId: parano.id, type: "paranoid_target",
        title: "🎯 Ta cible", body,
        mjTitle: "🎯 Paranoïaque",
  ...
  if (mouchard) {
    ...
    await notify({
      gameId, playerId: mouchard.id, type: "mouchard_setup",
      title: "📢 Mouchard", body,
      mjTitle: "📢 Mouchard",
  ...
  if (oracle) {
    const body = "À la première phase libre, prédis quelle faction (Civils, Méchants ou Neutres) remportera la partie. Tu gagneras avec elle si tu es en vie à la fin.";
    await notify({
      gameId, playerId: oracle.id, type: "oracle_setup",
      title: "🔮 Oracle", body,
      mjTitle: "🔮 Oracle",
  ...
  if (veuve) {
    const body = "À chaque rassemblement, choisis 2 cibles. Si l'une d'elles vote contre toi durant le vote suivant, les deux meurent au rassemblement d'après.";
    await notify({
      gameId, playerId: veuve.id, type: "veuve_setup",
      title: "🕷️ Veuve noire", body,
      mjTitle: "🕷️ Veuve noire", mjBody: `${veuve.pseudo} (Veuve noire) prête à désigner ses cibles au rassemblement.`,
    });
  ...
  if (cuisinier) {
    await grantItem(cuisinier.id, buildItem("couteau", {
      from: "Cuisine",
      originFaction: "Civil",
  ...
  if (stratege) {
    await grantItem(stratege.id, buildItem("couteau", {
      from: "Stratège",
      originFaction: "Méchant",
      nameOverride: "Couteau du Stratège",
      descriptionOverride: "Ton arme. Frappe immédiatement une cible, ou utilise ta capacité pour la marquer (mort différée d'un tour).",
      payload: { mechant_origin: true },
    }));
    const body = "Tu es le Stratège. Chaque phase libre, marque une cible (elle sera prévenue et mourra à l'annonce du tour suivant), ou frappe immédiatement avec ton couteau.";
    await notify({
      gameId, playerId: stratege.id, type: "stratege_setup",
      title: "♟️ Stratège — éveillé", body,
      mjTitle: "♟️ Stratège", mjBody: `${stratege.pseudo} (Stratège) reçoit un couteau et peut marquer ses cibles.`,
    });
  ...
  if (apo) {
    await grantItem(apo.id, buildItem("fiole_vie", { from: "Apothicairerie", originFaction: "Civil", payload: { apo_own: true } }));
    await grantItem(apo.id, buildItem("fiole_mort", { from: "Apothicairerie", originFaction: "Civil", payload: { apo_own: true } }));
    await grantItem(apo.id, buildItem("fiole_clairvoyance", { from: "Apothicairerie", originFaction: "Civil", payload: { apo_own: true } }));
  }
  ...
  if (conserv) {
    const body = "Une fois par phase libre ET une fois par rassemblement, désigne un joueur : il recevra une relique maudite au hasard. Tu gagnes si Le Cœur du Manoir est distribué.";
    await notify({
      gameId, playerId: conserv.id, type: "conservateur_setup",
      title: "🗝️ Conservateur — éveillé", body,
      mjTitle: "🗝️ Conservateur", mjBody: `${conserv.pseudo} (Conservateur) distribue des reliques aux autres joueurs.`,
    });
...
export async function tickPhase(gameId: string): Promise<void> {
  ...
  try {
    const { data: g } = await supabase
      ...
      .eq("id", gameId).single();
    const game = g as { current_phase: Phase; phase_started_at: string | null; phase_duration_s: number | null; status: string; paused?: boolean } | null;
...
export async function setPaused(gameId: string, paused: boolean): Promise<void> {
  await supabase.from("games").update({ paused } as never).eq("id", gameId);
}
...
export async function setForcedFrame(gameId: string, frame: string | null): Promise<void> {
  await supabase.from("games").update({ forced_frame: frame } as never).eq("id", gameId);
}

/** Termine la partie immédiatement avec un gagnant arbitraire (déclenché par fin spéciale). */
export async function endGameWithWinner(gameId: string, winner: string, reason: string): Promise<void> {
  ...
  const rows = (ps ?? []).map((p: { id: string }) => ({
    ...
    title: `🏆 ${winner} a gagné`, body: reason,
...
async function resolveCycleTransition(gameId: string) {
  ...
  for (const p of allPs) {
    ...
    if ((m.poison_resolves_cycle as number | undefined) === nextCycleN) {
      ...
      if (protUntil >= nextCycleN) {
        ...
        await notify({ gameId, playerId: p.id, type: "saved", title: "🛡️ Sauvé du poison", body: "Une protection t'a sauvé.", mjTitle: "🛡️ Poison neutralisé", mjBody: `${p.pseudo} a été sauvé du poison par une protection.` });
      } else {
        await patchMeta(p.id, { poison_resolves_cycle: null, poisoned: false });
  ...
  for (const p of allPs) {
    ...
    await notify({
      gameId, playerId: p.id, type: "released",
      title: "🔓 Libéré",
  ...
  for (const pup of allPs) {
    ...
    await supabase.from("player_statuses").insert({
      game_id: gameId, player_id: target.id, status_slug: "manipulated",
      source: "role:marionnettiste",
      active_from_tour: nextCycleN, active_until_tour: nextCycleN,
    });
    await notify({
      gameId, playerId: target.id, type: "manipulated",
...
async function autoPickMouchard(gameId: string, tour: number): Promise<void> {
  const { data: mRow } = await supabase
    .from("players").select().eq("game_id", gameId).eq("role_slug", "mouchard").maybeSingle();
  ...
  if (isFalsified(tMeta)) {
    await supabase.from("role_actions").insert({
      game_id: gameId, actor_player_id: mouchard.id, tour, phase: "free",
    ...
    await notify({
      gameId, playerId: mouchard.id, type: "mouchard_reveal",
      title: "📢 Mouchard (auto)", body: `Cible auto : ${t1.pseudo}. ${FALSIFIED_MSG}.`,
      mjTitle: "📢 Mouchard auto", mjBody: `${mouchard.pseudo} (Mouchard) → cible auto ${t1.pseudo} — piste falsifiée.`,
    });
    return;
  }
  const { data: rs } = await supabase.from("roles").select().eq("set_id", "set1");
...
async function autoPickOracle(gameId: string, tour: number): Promise<void> {
  ...
  const { data: oRow } = await supabase
    .from("players").select().eq("game_id", gameId).eq("role_slug", "oracle").maybeSingle();
  ...
  await supabase.from("role_actions").insert({
    game_id: gameId, actor_player_id: oracle.id, tour, phase: "free",
    payload: { effect: "prophecy", auto: true, faction } as never,
    result: { message: `Prophétie auto : ${faction}`, summary: `Prophétie auto : ${faction}` } as never,
  });

  await notify({
    gameId, playerId: oracle.id, type: "prophecy_set",
...
async function autoPickVengeur(gameId: string, tour: number): Promise<void> {
  ...
  const { data: vRow } = await supabase
    .from("players").select().eq("game_id", gameId).eq("role_slug", "vengeur").maybeSingle();
  ...
  await patchMeta(vengeur.id, { etre_cher: target.id, beloved_id: target.id, pending_beloved_choice: false });
...
async function autoPickUsurpateur(gameId: string, tour: number): Promise<void> {
  ...
  const { data: uRow } = await supabase
    .from("players").select().eq("game_id", gameId).eq("role_slug", "usurpateur").maybeSingle();
  ...
  await supabase.from("role_actions").insert({
    game_id: gameId, actor_player_id: u.id, tour, phase: "free",
    payload: { effect: "cover_pick", auto: true, cover: pick } as never,
    result: { message: `Couverture auto : ${label}`, summary: `Couverture auto : ${label}` } as never,
  });
  await notify({
    gameId, playerId: u.id, type: "cover_pending",
...
export async function ringGathering(gameId: string, reason = "MJ"): Promise<string> {
  ...
  const { data: gc, error } = await supabase
    .from("gathering_calls").insert({ game_id: gameId, tour, reason }).select().single();
  if (error) throw error;
  ...
  const { data: guetteurs } = await supabase.from("players").select().eq("game_id", gameId).eq("role_slug", "guetteur").eq("is_alive", true);
  for (const gu of ((guetteurs ?? []) as PlayerRow[])) {
    ...
    if (visitors.length > 0) {
      ...
      await notify({
        gameId, playerId: gu.id, type: "guetteur_visit",
...
async function deliverStrategeMarks(gameId: string, tour: number): Promise<void> {
  ...
  for (const p of ((ps ?? []) as PlayerRow[])) {
    ...
    const mark = m.targeted_by_stratege as { from_tour?: number; resolves_tour?: number } | undefined;
    if (!mark) continue;

    // Passe 1 — marque fraîche : prévient la cible.
    if (mark.from_tour === tour && p.is_alive) {
      await notify({
        gameId, playerId: p.id, type: "stratege_marked",
        title: "🎯 Tu es ciblé",
    ...
    if ((mark.resolves_tour ?? -1) <= tour && mark.from_tour !== tour) {
      if (p.is_alive) {
        await notify({
          gameId, playerId: p.id, type: "stratege_survived",
...
export async function castVote(gameId: string, voterId: string, targetId: string) {
  ...
  const { data: vp } = await supabase.from("players")
    .select("is_alive, is_imprisoned").eq("id", voterId).single();
  const voter = vp as { is_alive: boolean; is_imprisoned: boolean } | null;
  if (!voter || !voter.is_alive || voter.is_imprisoned) return;
  const { data: g } = await supabase.from("games").select("current_tour").eq("id", gameId).single();
  const tour = (g as { current_tour: number } | null)?.current_tour ?? 1;
  await supabase.from("votes").delete()
    .eq("game_id", gameId).eq("tour", tour).eq("voter_player_id", voterId);
  await supabase.from("votes").insert({
    game_id: gameId, tour, voter_player_id: voterId, target_player_id: targetId,
  });
...
export async function tallySuspicionVote(gameId: string): Promise<{
  ...
  const players = (ps ?? []) as Array<{ id: string; is_alive: boolean; is_imprisoned: boolean; is_mj: boolean; role_meta: Meta }>;
...
export async function closeVote(gameId: string) {
  ...
  const { data: gVar } = await supabase.from("games").select("variant" as never).eq("id", gameId).maybeSingle();
  ...
  for (const v of ((veuves ?? []) as PlayerRow[])) {
    const vm = meta(v);
    const pairs = ((vm.veuve_pairs as Array<{ tour: number; pair: string[] }> | undefined) ?? [])
    ...
    if (variant === "suspicion") {
      ...
      const { data: votesAgainst } = await supabase.from("votes")
        .select("voter_player_id").eq("game_id", gameId).eq("tour", tour).eq("target_player_id", v.id);
      for (const x of ((votesAgainst ?? []) as Array<{ voter_player_id: string }>)) voters.add(x.voter_player_id);
    }
    ...
    for (const tid of toKill) {
      await submitIntent({
        gameId, tour: tour + 1, phase: "free",
        actorId: v.id, targetId: tid,
        category: "ATTACK", timing: "DEFERRED",
        source: "role:veuve_noire",
        payload: { kill_reason: "veuve_noire_vote_trigger" },
      });
    }
    // Purge les paires consommées pour ne pas re-déclencher.
    const remaining = ((vm.veuve_pairs as Array<{ tour: number; pair: string[] }> | undefined) ?? [])
      .filter((p) => p.tour !== tour);
    await patchMeta(v.id, { veuve_pairs: remaining });
    await notify({
      gameId, playerId: v.id, type: "veuve_trigger",
      title: "🕷️ La toile se referme",
      body: `Un époux a voté contre toi. ${toKill.length} cible(s) mourront au prochain rassemblement.`,
      mjTitle: "🕷️ Veuve noire", mjBody: `${v.pseudo} (Veuve noire) déclenche la mort de ${toKill.length} époux (vote contre elle).`,
    });
    void allTargets;
  }
  ...
  if (targetId) {
    ...
    if (slug === "saint") {
      ...
      const rows = (ps ?? []).map((p: { id: string }) => ({
        game_id: gameId, player_id: p.id, type: "game_end",
        title: "🕯️ Le Saint a été condamné", body: "Défaite des Citoyens.",
    ...
    const broadcast = ((allPs ?? []) as Array<{ id: string }>).map((p) => ({
      game_id: gameId, player_id: p.id, type: "vote_result",
 ...
 */
export async function killPlayer(gameId: string, playerId: string, reason = "engine", attackerId?: string, extra?: Record<string, unknown>): Promise<boolean> {
  ...
  if (prot >= tour && reason !== "vote") {
    ...
    await notifyMJ({ gameId, type: "shielded", title: "🛡️ Attaque bloquée", body: `${player.pseudo} a été attaqué (${reason}) mais une protection l'a sauvé.` });
    // Saint block : notifie l'agresseur que sa cible était bénite
    if (m.blessed_by_saint === true && attackerId) {
      await notify({ gameId, playerId: attackerId, type: "saint_block", title: "✨ Cible bénite", body: "Cette cible est bénite, votre action ne fonctionne pas." });
    }
    ...
    const isMechantReason = reason === "tueur"
      || reason === "croque_mitaine";
    if (isMechantReason && typeof m.guarded_by === "string") {
      ...
      if (killerId) await killPlayer(gameId, killerId, "majordome_riposte");
        ...
        if (tueurRow) await killPlayer(gameId, (tueurRow as { id: string }).id, "majordome_riposte");
      }
    }
    return false;
  }
  ...
  const whenLabel = deathPhase === "free" ? "durant la phase libre"
                  : deathPhase === "gathering" ? "au rassemblement"
                  : deathPhase === "vote" ? "lors du vote"
                  : "";
  ...
  const killBroadcast = ((allForKill ?? []) as Array<{ id: string }>).map((row) => ({
    game_id: gameId, player_id: row.id, type: "death",
    title: `💀 Mort de ${player.pseudo}`,
    body: `Faction : ${publicFaction}.`,
    payload: {
      target_id: playerId, tour, phase: deathPhase, reason,
      deferred: deathPhase === "free", cleaned: cleanedBroadcast, faction: publicFaction,
  ...
  await notifyMJ({
    gameId, type: cleanedBroadcast ? "death_cleaned" : "death",
    title: `💀 Mort de ${player.pseudo}`,
    ...
    payload: { target_id: playerId, tour, phase: deathPhase, reason, deferred: deathPhase === "free", cleaned: cleanedBroadcast, faction: realFaction },
  });
  ...
  if (deathPhase === "free") {
    ...
    await patchMeta(playerId, {
      ...
      death_cycle: tour, death_phase: deathPhase, death_reason: reason, death_cleaned: cleanedBroadcast,
    });
    emit("kill_deferred", `💀 ${player.pseudo} condamné (mort différée: ${reason})`, { playerId, reason });
    return true;
  }
  ...
  await patchMeta(playerId, { death_cycle: tour, death_phase: deathPhase, death_reason: reason, death_cleaned: cleanedBroadcast });
 ...
 */
async function runDeathCascades(gameId: string, player: PlayerRow, reason: string, _cycle: number): Promise<void> {
  const m = meta(player);
  ...
  if (!cleaned) {
    ...
    if (legiste && (legiste as PlayerRow).is_alive) {
      ...
      await notify({
        gameId, playerId: (legiste as PlayerRow).id, type: "autopsy",
        title: "🩺 Autopsie",
  ...
  if (typeof m.linked_with === "string") {
    ...
    if (lk && lk.is_alive && !lkMeta.pending_death) {
      await notify({ gameId, playerId: linkedId, type: "linked_death", title: "💔 Lien rompu", body: `Mort de ${player.pseudo} — ton lien t'emporte aussi.`, mjTitle: "💔 Lien rompu", mjBody: `${lk.pseudo} suit ${player.pseudo} dans la mort (lien Entremetteur).` });
  ...
  if (vengeur && vengeur.is_alive && (meta(vengeur).etre_cher as string | undefined) === playerId) {
    ...
    const { grantItem, buildItem } = await import("./items");
    await grantItem(vengeur.id, buildItem("couteau", { from: "Vengeance", originFaction: "Civil", descriptionOverride: "Ton être cher est tombé. Utilise ce couteau pour te venger une seule fois." }));
    await notify({ gameId, playerId: vengeur.id, type: "vengeance", title: "⚔️ Vengeance", body: "Ton être cher n'est plus. Un couteau apparaît dans ton inventaire.", mjTitle: "⚔️ Vengeance", mjBody: `${vengeur.pseudo} (Vengeur) reçoit un couteau : son être cher (${player.pseudo}) est mort.` });
  }
...
async function flushPendingDeaths(gameId: string): Promise<void> {
  ...
  const { count: deathCount } = await supabase.from("notifications")
    .select("id", { count: "exact", head: true })
    .eq("game_id", gameId).eq("type", "death")
    .contains("payload", { tour } as never);

  if (pending.length === 0 && (deathCount ?? 0) === 0) {
    await notifyMJ({
      gameId, type: "mj_announce",
      title: "🌅 Annonce",
...
async function isGenericMechantKiller(slug: string | null | undefined): Promise<boolean> {
  ...
  const { data } = await supabase.from("roles").select("faction, type").eq("slug", slug).maybeSingle();
...
async function promoteAcolyteToTueur(gameId: string, temporary: boolean): Promise<void> {
  ...
  const { data: rs } = await supabase.from("roles").select("slug, type, faction").in("slug", acolytes.map((a) => a.role_slug ?? ""));
  const acoInfo = new Map((rs ?? []).map((r: { slug: string; type: string; faction: string }) => [r.slug, r]));
  ...
  await patchMeta(heir.id, { promoted_from_acolyte: true, original_slug: originalSlug, temp_promotion: temporary });
  await notify({
    gameId, playerId: heir.id, type: "succession",
...
async function revertTempPromotion(gameId: string): Promise<void> {
  ...
  for (const p of ((promoted ?? []) as PlayerRow[])) {
    ...
    if (mp.temp_promotion === true && typeof mp.original_slug === "string") {
      ...
      await patchMeta(p.id, { promoted_from_acolyte: null, original_slug: null, temp_promotion: null });
      await notify({
        gameId, playerId: p.id, type: "succession_end",
 ...
 */
async function promoteVampireHeir(gameId: string, absentVampireId: string, temporary = false): Promise<void> {
 ...
 */
async function applyVampireConversion(gameId: string, vampireId: string, targetId: string, tour: number): Promise<boolean> {
  ...
  await notify({
    gameId, playerId: targetId, type: "converted",
    title: "🧛 Morsure de vampire", body: "Tu rejoins les Vampires. Tu connais les autres vampires.",
    mjTitle: "🧛 Conversion Vampire", mjBody: `${tPseudo} rejoint les Vampires.`,
  });
  ...
  const clan = ((ps ?? []) as PlayerRow[]).filter(
    (p) => p.id !== targetId && (p.role_slug === "vampire" || (p.role_meta as Record<string, unknown> | null)?.converted === true),
  );
  for (const v of clan) {
    await notify({ gameId, playerId: v.id, type: "vampire_clan", title: "🧛 Nouveau vampire", body: `${tPseudo} rejoint le clan.`, mjTitle: "🧛 Clan vampire", mjBody: `${v.pseudo} est notifié de l'arrivée de ${tPseudo}.` });
  }
  if (clan.length > 0) {
    await notify({ gameId, playerId: targetId, type: "vampire_clan_list", title: "🧛 Ton clan", body: clan.map((v) => v.pseudo).join(", "), mjTitle: "🧛 Liste clan", mjBody: `${tPseudo} reçoit la liste : ${clan.map((v) => v.pseudo).join(", ")}.` });
  }
  ...
  if (!chasseurEverExisted) {
    ...
    await notifyMJ({ gameId, type: "mj_announce", title: "🧛 1ère morsure", body: "Un joueur vient d'être mordu — un Chasseur de Vampire émerge." });
    ...
    const roleBySlug = new Map(((rs ?? []) as Array<{ slug: string; faction: string; type: string; is_special: boolean | null }>).map((r) => [r.slug, r]));
    ...
    if (pick) {
      ...
      await notify({ gameId, playerId: pick.id, type: "role_swap", title: "🩸 Tu sens l'appel", body: "Tu deviens Chasseur de Vampire. Traque-les avant qu'il ne soit trop tard.", mjTitle: "🩸 Chasseur émerge", mjBody: `${pick.pseudo} devient Chasseur de Vampire (choix aléatoire, 1ère morsure).` });
    }
  }
...
async function promoteAcolyteToStratege(gameId: string, temporary: boolean, sourceMeta: Meta): Promise<void> {
  const { data: aco } = await supabase.from("players").select().eq("game_id", gameId).eq("is_alive", true).eq("is_imprisoned", false);
  const acolytes = ((aco ?? []) as PlayerRow[]).filter((x) => {
    const mx = meta(x);
  ...
  await notify({
    gameId, playerId: heir.id, type: "succession_stratege",
...
async function revertTempStrategePromotion(gameId: string): Promise<void> {
  ...
  for (const p of ((promoted ?? []) as PlayerRow[])) {
    ...
    if (mp.temp_promotion === true && typeof mp.original_slug === "string") {
      ...
      await supabase.from("players").update({ role_slug: original }).eq("id", p.id);
      await patchMeta(p.id, {
        promoted_from_acolyte: null, original_slug: null, temp_promotion: null,
        fideles: null, fideles_ordered: null, stratege_pending_order: null, stratege_kills_done: null,
      });
      await notify({
        gameId, playerId: p.id, type: "succession_end",
        title: "↩️ Tu reprends ton rôle",
...
export async function imprisonPlayer(gameId: string, playerId: string, reason = "vote"): Promise<boolean> {
  ...
  if (m.immortal === true) {
    emit("imprison_blocked", `${(p as { pseudo: string } | null)?.pseudo} est immortel`, { playerId });
    return false;
  }
  ...
  emit("imprison", `🔒 ${(p as { pseudo: string } | null)?.pseudo} emprisonné (${reason})`, { playerId, reason });
  const prisonerName = (p as { pseudo: string } | null)?.pseudo ?? "?";
  await notify({ gameId, playerId, type: "imprisoned", title: "🔒 Tu es en prison", body: `Cause : ${reason}`, mjTitle: "🔒 Emprisonnement", mjBody: `${prisonerName} est emprisonné (cause : ${reason}).` });
  ...
  // Vengeur : l'être cher est emprisonné → débloquer la vengeance également.
  {
    ...
    if (vengeur && vengeur.is_alive && !vengeur.is_imprisoned) {
      ...
      if ((vm.etre_cher as string | undefined) === playerId && vm.kill_unlocked !== true) {
        await patchMeta(vengeur.id, { kill_unlocked: true });
        const { grantItem, buildItem } = await import("./items");
        await grantItem(vengeur.id, buildItem("couteau", { from: "Vengeance", originFaction: "Civil", descriptionOverride: "Ton être cher est emprisonné. Utilise ce couteau pour te venger une seule fois." }));
        await notify({ gameId, playerId: vengeur.id, type: "vengeance", title: "⚔️ Vengeance", body: `Ton être cher (${prisonerName}) a été emprisonné. Un couteau apparaît dans ton inventaire.`, mjTitle: "⚔️ Vengeance", mjBody: `${vengeur.pseudo} (Vengeur) reçoit un couteau : son être cher (${prisonerName}) est emprisonné.` });
      }
    }
  }
...
export async function releasePlayer(gameId: string, playerId: string) {
  ...
  if ((await isGenericMechantKiller((p as { role_slug?: string } | null)?.role_slug)) && pm.temp_promotion !== true) {
    await revertTempPromotion(gameId);
  }
  // Stratège d'origine libéré → annule la promotion temporaire éventuelle
  if ((p as { role_slug?: string } | null)?.role_slug === "stratege" && pm.temp_promotion !== true) {
...
export async function executeCapability(opts: {
  ...
  if (!allowedPhases.has(gp.current_phase)) {
    const msg = allowedPhases.has("gathering") && !allowedPhases.has("free")
  ...
  const isPuppetCall = opts.extra?.__puppet_call === true
    && (m.manipulated_by as string | undefined) === (opts.extra?.__puppeteer_id as string | undefined)
    && (m.manipulated_tour as number | undefined) === opts.tour;
  if (blocked && role.phase_activation !== "Permanent" && !(isPuppetCall && blocked === "Capacité bloquée")) {
  ...
  const log = (payload: Record<string, unknown> = {}) =>
    logCapability({
      gameId: opts.gameId, actorId: actor.id,
      targetId: t1?.id, targetId2: t2?.id,
      tour: opts.tour, phase: opts.phase,
  ...
  for (const tgt of opts.targets) {
    ...
    const { data: tFresh } = await supabase.from("players").select("role_meta").eq("id", tgt.id).single();
  ...
  switch (slug) {
    ...
    case "tueur": {
      ...
      const { data: acolytes } = await supabase
        ...
        .eq("is_alive", true);
      const teammates = ((acolytes ?? []) as Array<{ id: string; role_slug: string | null }>)
        .filter((p) => p.id !== actor.id && opts.rolesBySlug.get(p.role_slug ?? "")?.faction === "Méchant");
      for (const tm of teammates) {
        await notify({
          gameId: opts.gameId, playerId: tm.id, type: "killer_targeted",
          title: "🎯 Le Tueur a ciblé", body: `${t1.pseudo} est la cible de cette nuit.`,
        });
      }
      // V2 : on POSE une intention DEFERRED. Le resolver la traite au rassemblement,
      // en revérifiant protection / blocage / cible vivante.
      await submitIntent({
        gameId: opts.gameId, tour: opts.tour, phase: opts.phase,
        actorId: actor.id, targetId: t1.id,
        category: "ATTACK", timing: "DEFERRED",
        source: "role:tueur",
    ...
    case "vengeur": {
      ...
      if (m.kill_unlocked === true) {
        return { ok: false, message: "Vengeance débloquée — utilise ton couteau depuis l'inventaire." };
      }
      return { ok: false, message: "Capacité passive — si ton être cher meurt, un couteau apparaît dans ton inventaire." };
    }
    case "executeur": {
      if (!t1) return { ok: false, message: "Cible requise" };
      ...
      if (opts.tour - since < 1) return { ok: false, message: "Attends qu'elle ait passé 1 tour complet en prison." };
      ...
      if (ok) {
        ...
        const rows = ((allP ?? []) as Array<{ id: string }>).map((row) => ({
          game_id: opts.gameId, player_id: row.id, type: "execution_reveal",
          title: `⚖️ Exécution de ${t1.pseudo}`,
          body: `Rôle révélé : ${roleLabel}${execRole ? ` — ${execRole.faction}` : ""}.`,
          payload: { target_id: t1.id, role_slug: t1.role_slug, faction: execRole?.faction ?? null, tour: opts.tour } as never,
        }));
        if (rows.length) await supabase.from("notifications").insert(rows);
      }
    ...
    case "cuisinier": {
      return { ok: false, message: "Capacité passive — utilise ton couteau depuis l'inventaire." };
    }
    ...
    case "armurier": {
      ...
      const { grantItem, buildItem } = await import("./items");
      await grantItem(t1.id, buildItem("couteau", {
        from: "Inconnu",
        originFaction: "Méchant",
        nameOverride: "Couteau de l'Armurier",
        descriptionOverride: "Un couteau anonyme remis par l'Armurier apparaît dans ton inventaire. Tu peux l'utiliser une fois pour tuer un joueur — résolu au rassemblement suivant.",
        ...
        payload: { mechant_origin: true, gifted_by_id: actor.id, gifted_by_pseudo: actor.pseudo },
      }));
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "anon_gift",
    ...
    case "empoisonneur": {
      ...
      await submitIntent({
        ...
        category: "CASCADE", timing: "DEFERRED",
    ...
    case "vampire": {
      ...
      if (tRole?.slug === "chasseur_de_vampire") {
        ...
        return { ok: false, message: "Le Chasseur de Vampire est immunisé — ta morsure est perdue." };
      }
      ...
      await submitIntent({
        gameId: opts.gameId, tour: opts.tour, phase: opts.phase,
        actorId: actor.id, targetId: t1.id,
        category: "CONVERT", timing: "DEFERRED",
        source: "role:vampire",
        payload: { target_pseudo: t1.pseudo },
      });
      return { ok: true, message: `Morsure programmée sur ${t1.pseudo} — résolue au prochain rassemblement` };
    }
    ...
    case "chasseur_de_vampire": {
      ...
      return { ok: true, message: isVamp ? `🔴 ${t1.pseudo} EST un vampire — exécution programmée au rassemblement` : `🟢 ${t1.pseudo} n'est pas un vampire`, reveal: { isVampire: isVamp } };
    }
    ...
    case "assistant_du_detective": {
      ...
      const compatible = (a: FT, b: FT): boolean => {
        ...
        if (a.faction === "Neutre") {
          ...
          if (a.type === "BÉNIN" && (b.type === "PROTECTEUR" || b.type === "SUPPORT")) return true;
        }
        return false;
      };

      const EXCLUDED_SLUGS = new Set(["detective", "assistant_du_detective"]);
      const isPickable = (r: { slug: string; type: string; is_special: boolean | null; emergent: boolean | null }) =>
        !EXCLUDED_SLUGS.has(r.slug) && !r.is_special && !r.emergent && r.type !== "BOULET";

      const targetFT: FT = { faction: trueRole?.faction ?? "Civil", type: trueRole?.type ?? "SUPPORT" };
      const allRoles = Array.from(opts.rolesBySlug.values()).filter((r) => r.slug !== trueSlug && isPickable(r));

      // Pool primaire : types compatibles selon le mapping.
      let decoyPool = allRoles.filter((r) => compatible(targetFT, { faction: r.faction, type: r.type }));
      // Fallback 1 : même faction (peu importe le type, hors BOULET).
      if (decoyPool.length < 2) {
        const extra = allRoles.filter((r) => r.faction === targetFT.faction && !decoyPool.includes(r));
    ...
    case "majordome": {
      ...
      const { data: existingProtect } = await supabase.from("role_actions")
        .select("id")
        .eq("game_id", opts.gameId)
      ...
      await submitIntent({
        gameId: opts.gameId, tour: opts.tour, phase: opts.phase,
        actorId: actor.id, targetId: t1.id,
        category: "PROTECT", timing: "DEFERRED",
        source: "role:majordome",
        payload: { target_pseudo: t1.pseudo },
      });
      await used({ effect: "butler_intent", target: t1.id });
      await notifyMJ({ gameId: opts.gameId, type: "protected", title: "🛡️ Protection Majordome", body: `${actor.pseudo} (Majordome) protège ${t1.pseudo} — résolu au rassemblement.` });
    ...
    case "babysitter": {
      ...
      await submitIntent({
        gameId: opts.gameId, tour: c, phase: opts.phase,
        actorId: actor.id, targetId: t1.id,
        category: "PROTECT", timing: "DEFERRED",
      ...
      await notify({ gameId: opts.gameId, playerId: t1.id, type: "babysat", title: "🧸 Babysitter", body: "Au prochain tour, tu seras à l'abri de la mort mais ta capacité sera bloquée.", mjTitle: "🧸 Babysitter", mjBody: `${actor.pseudo} (Babysitter) gardera ${t1.pseudo} au prochain tour.` });
    ...
    case "ange_gardien": {
      ...
      await submitIntent({
        gameId: opts.gameId, tour: opts.tour, phase: opts.phase,
        actorId: actor.id, targetId: target,
        category: "PROTECT", timing: "DEFERRED",
    ...
    case "saint": {
      ...
      return { ok: true, message: `${target.pseudo} béni — protection pendant 2 tours complets.` };
    }
    ...
    case "marionnettiste": {
      ...
      })) return { ok: false, message: `${t1.pseudo} est sous bénédiction — manipulation annulée.` };
      ...
      return { ok: true, message: `Au prochain tour, tu prendras le contrôle de la capacité de ${t1.pseudo}.` };
    }
    ...
    case "maitre_chanteur": {
      ...
      await patchMeta(t1.id, { blackmail_from_cycle: opts.tour + 1, blackmail_until_cycle: opts.tour + 1 });
      await used({ effect: "blackmail" });
      await notify({ gameId: opts.gameId, playerId: t1.id, type: "blackmail", title: "🤐 Sous chantage", body: "Demain, tu ne pourras pas agir.", mjTitle: "🤐 Chantage", mjBody: `${actor.pseudo} (Maître chanteur) fait chanter ${t1.pseudo} pour le prochain tour.` });
    ...
    case "cleaner": {
      ...
      if (next) {
        await notify({
          gameId: opts.gameId, playerId: actor.id, type: "clean_armed",
          title: "🧹 Effaceur armé", body: "Le prochain meurtre du Tueur passera inaperçu.",
          mjTitle: "🧹 Cleaner", mjBody: `${actor.pseudo} (Cleaner) arme l'effaçage du prochain meurtre du Tueur.`,
        });
      }
    ...
    case "usurpateur": {
      ...
      return { ok: true, message: r ? `Tu apparais comme ${r.icon} ${r.name_fr}` : "Couverture en place" };
    }
    ...
    case "entremetteur": {
      ...
      if (t1.id === actor.id || t2.id === actor.id) return { ok: false, message: "Tu ne peux pas te lier toi-même." };
      ...
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "linked_partner",
        title: "💞 Lien noué", body: `Ton âme sœur secrète : ${t2.pseudo}. Si l'un meurt, l'autre suit.`,
        mjTitle: "💞 Lien", mjBody: `Lien noué entre ${t1.pseudo} et ${t2.pseudo}.`,
      });
      await notify({
        gameId: opts.gameId, playerId: t2.id, type: "linked_partner",
        title: "💞 Lien noué", body: `Ton âme sœur secrète : ${t1.pseudo}. Si l'un meurt, l'autre suit.`,
        mjTitle: "💞 Lien", mjBody: `Lien noué entre ${t2.pseudo} et ${t1.pseudo}.`,
      });
      await notifyMJ({
        gameId: opts.gameId, type: "link",
        title: "💞 Liens tissés",
    ...
    case "facteur": {
      ...
      const { grantItem, buildItem } = await import("./items");
      await grantItem(t1.id, buildItem("lettre", {
        from: "Anonyme",
        descriptionOverride: "Une lettre anonyme t'a été remise.",
      }));
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "letter",
    ...
    case "croque_mitaine": {
      ...
      await submitIntent({
        gameId: opts.gameId, tour: opts.tour, phase: opts.phase,
        actorId: actor.id, targetId: pick.id,
        category: "ATTACK", timing: "DEFERRED",
        source: "role:croque_mitaine",
        payload: { kill_reason: "croque_mitaine", target_pseudo: pick.pseudo, spared_id: other.id, spared_pseudo: other.pseudo, mechant_mechanic: true },
      });
      // Notifie le survivant : il a senti le souffle du Croque-mitaine.
      await notify({
        gameId: opts.gameId, playerId: other.id, type: "boogey_breath",
        title: "👻 Un souffle glacé",
        body: "Tu as senti son souffle dans ton dos… le Croque-mitaine t'a frôlé.",
        mjTitle: "👻 Croque-mitaine", mjBody: `${other.pseudo} a été épargné de justesse par le Croque-mitaine.`,
      });
      await used({ effect: "kill_one_of_two_intent", picked: pick.id, spared: other.id });
      return { ok: true, message: `${pick.pseudo} : attaque programmée — ${other.pseudo} épargné` };
    }
    case "veuve_noire": {
      ...
      await notify({
        gameId: opts.gameId, playerId: actor.id, type: "veuve_pair",
    ...
    case "heritier_dechu": {
      ...
      const verdict = override
        ?? (t1.role_slug === "usurpateur" ? "suspicious" : (r?.police_verdict ?? "na"));
      const isSuspect = verdict === "suspicious";
      await used({ effect: "heir_inquiry", target: t1.id, verdict });
      await notify({
        gameId: opts.gameId, playerId: actor.id, type: "heir_inquiry",
        title: "👑 Héritier — Enquête",
        body: isSuspect ? `🟠 ${t1.pseudo} éveille les soupçons.` : `🟢 Rien à signaler sur ${t1.pseudo}.`,
        mjTitle: "👑 Héritier déchu",
        mjBody: `${actor.pseudo} (Héritier déchu) enquête sur ${t1.pseudo} → ${verdict}.`,
      });
      return { ok: true, message: isSuspect ? `🟠 ${t1.pseudo} : soupçons` : `🟢 ${t1.pseudo} : rien à signaler` };
    }
    case "parieur_tricheur": {
      ...
      await submitIntent({
        gameId: opts.gameId, tour: opts.tour, phase: opts.phase,
        actorId: actor.id, targetId: loserId,
        category: "ATTACK", timing: "DEFERRED",
      ...
      await supabase.from("player_statuses").insert({
        game_id: opts.gameId, player_id: loserId, status_slug: "dice_loser",
        source: "role:parieur_tricheur",
        active_from_tour: opts.tour, active_until_tour: opts.tour,
        payload: {} as never,
      });
      ...
      const duelPayload = {
        duelId,
        actorId: actor.id, actorPseudo: actor.pseudo,
        targetId: t1.id, targetPseudo: t1.pseudo,
        rounds, loserId, winnerId,
      };
      const duelBody = `Toi : ${meBest} (3d6) · ${t1.pseudo} : ${themRoll}. ${loserPseudo} mourra à la prochaine annonce.`;
      await notify({
        gameId: opts.gameId, playerId: actor.id, type: "dice_duel",
        title: meLoses ? "🎲 Pari perdu" : "🎲 Pari gagné",
        body: duelBody, payload: duelPayload,
      });
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "dice_duel",
        title: meLoses ? "🎲 Duel gagné" : "🎲 Duel perdu",
        body: `${actor.pseudo} : ${meBest} (3d6) · toi : ${themRoll}. ${loserPseudo} mourra à la prochaine annonce.`,
        payload: duelPayload,
      });
      await notifyMJ({
        gameId: opts.gameId, type: "dice_duel",
      ...
      return { ok: !meLoses, pending: true, message: meLoses ? `🎲 ${meBest} < ${themRoll} — tu perds le pari` : `🎲 ${meBest} > ${themRoll} — ${t1.pseudo} perd le pari` };
    }
    ...
    case "apothicaire": {
      if (!t1) return { ok: false, message: "Cible requise" };
      if (t1.id === actor.id) return { ok: false, message: "Offre la fiole à un autre joueur (pour l'utiliser toi-même, passe par ton Carnet)." };
      ...
      const available: Array<"heal" | "poison" | "reveal"> = (["heal", "poison", "reveal"] as const).filter((f) => !flasksUsed.includes(f));
      if (available.length === 0) return { ok: false, message: "Toutes tes fioles ont déjà été utilisées." };
      const fiole = requested && available.includes(requested) ? requested : available[0];
      const { grantItem, buildItem } = await import("./items");
      const slugMap = { heal: "fiole_vie", poison: "fiole_mort", reveal: "fiole_clairvoyance" } as const;
      const targetSlug = slugMap[fiole];
      // Textes du cadeau : nomment le RÔLE (« l'Apothicaire ») mais jamais le joueur.
      const fioleNames = { fiole_vie: "Fiole de vie", fiole_mort: "Fiole de mort", fiole_clairvoyance: "Fiole de clairvoyance" } as const;
      const giftDesc = {
        fiole_vie: "💚 Tu as reçu une Fiole de vie de l'Apothicaire. Utilise-la depuis ton Carnet pour protéger un joueur jusqu'au prochain rassemblement.",
        fiole_mort: "☠️ Tu as reçu une Fiole de mort de l'Apothicaire. Utilise-la depuis ton Carnet pour empoisonner une cible — elle mourra au prochain rassemblement.",
        fiole_clairvoyance: "🔮 Tu as reçu une Fiole de clairvoyance de l'Apothicaire. Utilise-la depuis ton Carnet sur un joueur pour découvrir, toi seul, sa faction.",
      } as const;
      // Dépose la fiole (utilisable librement) dans l'inventaire du receveur.
      await grantItem(t1.id, buildItem(targetSlug, { from: "Apothicaire", originFaction: "Civil", descriptionOverride: giftDesc[targetSlug] }));
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "fiole_offerte",
        title: "🎁 Une fiole t'est offerte", body: giftDesc[targetSlug],
        mjTitle: "🎁 Apothicaire", mjBody: `${actor.pseudo} (Apothicaire) offre une ${fioleNames[targetSlug]} à ${t1.pseudo}.`,
      });
    ...
    case "policier": {
      ...
      return { ok: true, message: verdict === "suspicious" ? `🟠 ${t1.pseudo} : soupçons` : `🟢 ${t1.pseudo} : rien à signaler` };
    }
    ...
    case "imitateur": {
      ...
      deads.sort((a, b) => {
        const da = (((a.role_meta ?? {}) as Record<string, unknown>).death_cycle as number | undefined) ?? 0;
        const db = (((b.role_meta ?? {}) as Record<string, unknown>).death_cycle as number | undefined) ?? 0;
        return db - da;
      });
      const last = deads[0];
      if (!last) return { ok: false, message: "Aucun mort à imiter." };
      if (last.id !== t1.id) return { ok: false, message: `Tu ne peux imiter que le dernier mort (${last.pseudo}).` };
      const r = opts.rolesBySlug.get(t1.role_slug ?? "");
      if (!r) return { ok: false, message: "Cible sans rôle" };
      await patchMeta(actor.id, { imitated_slug: r.slug, original_slug: actor.role_slug ?? "imitateur" });
      await supabase.from("players").update({ role_slug: r.slug }).eq("id", actor.id);
      await used({ effect: "imitate", slug: r.slug });
      await notify({ gameId: opts.gameId, playerId: actor.id, type: "imitate", title: "🎭 Imitation", body: `Tu deviens : ${r.icon} ${r.name_fr}. Sa capacité est désormais la tienne.`, mjTitle: "🎭 Imitateur", mjBody: `${actor.pseudo} (Imitateur) copie ${t1.pseudo} (${r.icon} ${r.name_fr}) — slug basculé.` });
    ...
    case "barman": {
      ...
      await submitIntent({
        gameId: opts.gameId, tour: protectTour, phase: "free",
        actorId: actor.id, targetId: picked.id,
        category: "PROTECT", timing: "DEFERRED",
        source: "role:barman",
      ...
      await patchMeta(picked.id, {
        drunk_until_cycle: protectTour, drunk_from_cycle: protectTour,
        blocked_until_cycle: protectTour, blocked_from_cycle: protectTour,
      });
      ...
      await used({ effect: "barman_round", picked: picked.id, other: other.id, pair: [t1.id, t2.id] });
      await notify({
        gameId: opts.gameId, playerId: picked.id, type: "drunk",
        title: "🍺 Ivre", body: "Le barman t'a servi un verre de trop. Ta capacité sera bloquée demain.",
        mjTitle: "🍺 Barman", mjBody: `${actor.pseudo} (Barman) sert ${t1.pseudo} & ${t2.pseudo} — ${picked.pseudo} tombe ivre (et protégé).`,
      });
      await notify({
        gameId: opts.gameId, playerId: other.id, type: "good_time",
        title: "🍹 Bon moment", body: `Tu as passé du bon temps avec ${picked.pseudo}.`,
      });
      await notifyMJ({
        gameId: opts.gameId, type: "protected",
        title: "🛡️ Protection Barman", body: `${actor.pseudo} (Barman) protège ${picked.pseudo} (ivre). ${other.pseudo} passe un bon moment avec ${picked.pseudo}.`,
      });
      return { ok: true, message: `${t1.pseudo} & ${t2.pseudo} — l'un ivre & à l'abri, l'autre passe un bon moment` };
    }
    ...
    case "juge": {
      ...
      await patchMeta(t1.id, { pending_release_for_cycle: opts.tour + 1, pending_release_by: actor.id });
      await used({ effect: "judge_release_scheduled" });
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "release_scheduled",
      ...
      return { ok: true, message: `Libération de ${t1.pseudo} programmée pour le tour ${opts.tour + 1}` };
    }
    ...
    case "oracle": {
      ...
      await notify({
        gameId: opts.gameId, playerId: actor.id, type: "prophecy_set",
    ...
    case "guetteur": {
      await log({ effect: "passive_use" });
      await notify({
        gameId: opts.gameId, playerId: actor.id, type: "passive_ack",
        title: "📌 Capacité passive",
        body: "Ton rôle agit en arrière-plan — surveille ton carnet pour les révélations.",
        mjTitle: `📌 Passif ${slug}`, mjBody: `${actor.pseudo} (${slug}) — capacité passive activée.`,
      });
      return { ok: true, message: "Capacité passive — voir notifications" };
    }
    ...
    case "mouchard": {
      ...
      if (isFalsified(meta(t1))) {
        ...
        await notify({
          gameId: opts.gameId, playerId: actor.id, type: "mouchard_reveal",
          title: "📢 Mouchard", body: FALSIFIED_MSG,
          mjTitle: "📢 Mouchard", mjBody: `${actor.pseudo} (Mouchard) cible ${t1.pseudo} — piste falsifiée.`,
        });
        return { ok: true, message: FALSIFIED_MSG };
      }
    ...
    case "stratege": {
      if (!t1) return { ok: false, message: "Cible requise" };
      if (t1.id === actor.id) return { ok: false, message: "Tu ne peux pas te désigner toi-même." };
      await submitIntent({
        gameId: opts.gameId, tour: opts.tour + 1, phase: opts.phase,
        actorId: actor.id, targetId: t1.id,
        category: "ATTACK", timing: "DEFERRED",
        source: "role:stratege",
        payload: { kill_reason: "stratege_embuscade", target_pseudo: t1.pseudo, mechant_mechanic: true },
      });
      await patchMeta(t1.id, {
        targeted_by_stratege: { from_tour: opts.tour, resolves_tour: opts.tour + 1, stratege_id: actor.id },
      });
      // Prévient l'équipe Méchants (comme le Tueur).
      const { data: team } = await supabase
        .from("players").select("id, role_slug")
        .eq("game_id", opts.gameId).eq("is_alive", true);
      const teammates = ((team ?? []) as Array<{ id: string; role_slug: string | null }>)
        .filter((p) => p.id !== actor.id && opts.rolesBySlug.get(p.role_slug ?? "")?.faction === "Méchant");
      for (const tm of teammates) {
        await notify({
          gameId: opts.gameId, playerId: tm.id, type: "killer_targeted",
          title: "🎯 Le Stratège a marqué", body: `${t1.pseudo} mourra à la prochaine annonce.`,
        });
      }
      await used({ effect: "stratege_mark", target: t1.id });
      await notifyMJ({
        gameId: opts.gameId, type: "mj_announce",
        title: "♟️ Stratège — marque",
        body: `${actor.pseudo} (Stratège) marque ${t1.pseudo} — mort prévue à l'annonce du tour ${opts.tour + 1}.`,
      });
      return { ok: true, message: `🎯 ${t1.pseudo} marqué — il mourra à l'annonce du prochain tour.` };
    }
    ...
    case "voleur": {
      ...
      if (inv.length === 0) {
        ...
        await notify({
          gameId: opts.gameId, playerId: actor.id, type: "steal_empty",
          title: "🥷 Inventaire vide",
          body: `${t1.pseudo} n'avait aucun objet à voler.`,
          mjTitle: "🥷 Voleur", mjBody: `${actor.pseudo} (Voleur) tente de voler ${t1.pseudo} — inventaire vide.`,
        });
      ...
      const stolenItem = { ...stolen, received_from: t1.pseudo, received_at: new Date().toISOString() };
      ...
      await notify({
        gameId: opts.gameId, playerId: actor.id, type: "steal_ok",
        title: "🥷 Vol réussi", body: `Tu dérobes ${itemName} à ${t1.pseudo}.`,
        mjTitle: "🥷 Voleur", mjBody: `${actor.pseudo} (Voleur) dérobe ${itemName} à ${t1.pseudo}.`,
      });
      if (t1.is_alive) {
        await notify({
          gameId: opts.gameId, playerId: t1.id, type: "stolen_from",
          title: "🥷 Vol", body: `On t'a dérobé ${itemName}.`,
        });
      }
    ...
    case "conservateur": {
      ...
      if (variant === "coeur_du_manoir") {
        ...
        await endGameWithWinner(opts.gameId, "Conservateur",
          `${actor.pseudo} (Conservateur) a confié ${def.icon} ${def.name} à ${t1.pseudo}. Le Manoir reconnaît son gardien.`);
        return { ok: true, message: `🫀 Tu as offert Le Cœur du Manoir à ${t1.pseudo} — Victoire du Conservateur.` };
      }
      ...
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "relique_received",
      ...
      await notify({
        gameId: opts.gameId, playerId: actor.id, type: "relique_given",
    ...
    case "accusateur": {
      ...
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "accused",
        title: "🔖 Accusation contre toi", body: `Un Accusateur a jeté la suspicion sur toi (1 tour).`,
        mjTitle: "🔖 Accusateur", mjBody: `${actor.pseudo} (Accusateur) accuse ${t1.pseudo} — suspect pour 1 tour.`,
      });
      await notify({
        gameId: opts.gameId, playerId: actor.id, type: "accuse_ok",
        title: "🔖 Accusation lancée", body: `Suspicion jetée sur ${t1.pseudo} (1 tour).`,
      });
    ...
    case "falsificateur": {
      if (!t1) return { ok: false, message: "Cible requise" };
      if (t1.id === actor.id) return { ok: false, message: "Tu ne peux pas te falsifier toi-même." };
      if (await tryBlessingBlock({
        gameId: opts.gameId, tour: opts.tour, phase: opts.phase,
        actor: { id: actor.id, pseudo: actor.pseudo },
        actorRole: { faction: role.faction, type: role.type ?? null },
      ...
      })) return { ok: false, message: `${t1.pseudo} est sous bénédiction — falsification annulée.` };
      const tMeta = meta(t1);
      if (isFalsified(tMeta)) {
        await used({ effect: "falsify_redundant", target: t1.id });
        return { ok: true, message: `Piste déjà falsifiée sur ${t1.pseudo}.` };
      }
      await patchMeta(t1.id, { falsified: true, falsified_by: actor.id, falsified_at_tour: opts.tour });
      ...
      await notify({
        gameId: opts.gameId, playerId: t1.id, type: "falsified",
      ...
      return { ok: true, message: `Piste falsifiée sur ${t1.pseudo} — les investigateurs ne pourront plus rien apprendre sur cette cible.` };
    }
  ...
  if (dispatchResult.ok) {
    ...
    if (forcedBy && forcedCycle === opts.tour) {
      ...
      await notify({
        gameId: opts.gameId, playerId: forcedBy, type: "puppet_mirror",
...
const BOT_PSEUDOS_POOL = [
  "Bot Alice", "Bot Bob", "Bot Cleo", "Bot Dré", "Bot Émile", "Bot Faye",
  "Bot Gus", "Bot Hana", "Bot Ivo", "Bot Jin", "Bot Kya", "Bot Léo",
  "Bot Mia", "Bot Nio", "Bot Ola", "Bot Pim",
];
...
function buildBotRows(gameId: string, existing: ExistingBotInfo[], count: number) {
  ...
  const usedAvatars = new Set(
    existing
      .map((p) => p.role_meta?.avatar as string | undefined)
...
export async function addBotPlayers(gameId: string, count: number): Promise<number> {
  ...
  if (added > 0) {
    emit("bot_added", `🤖 ${added} bot${added > 1 ? "s" : ""} ajouté${added > 1 ? "s" : ""}`, { gameId });
  }
  return added;
}
...
export async function resetGame(gameId: string) {
  ...
  await supabase.from("players")
    ...
    .eq("game_id", gameId);
  await supabase.from("games").update({
    status: "lobby", current_phase: "lobby", current_tour: 0, started_at: null, ended_at: null,
  }).eq("id", gameId);
