...
async function evaluateWin(gameId) {
  ...
  const isBlockingNeutre = (p) => {
    ...
    if ((r.type ?? "").toUpperCase() === "B\xC9NIN") return false;
  ...
  const isBenignNeutre = (p) => {
    const r = p.role_slug ? rolesBySlug.get(p.role_slug) : null;
    return r?.faction === "Neutre" && (r?.type ?? "").toUpperCase() === "B\xC9NIN";
  };
...
function isHostileFactionType(faction, type) {
  if (faction === "M\xE9chant") return true;
...
async function drawRoles(playerCount, modeDetectivePlayer, bannedSlugs = []) {
  ...
  const slugs = [];
  const tueurs = eligible.filter((r) => r.type === "TUEUR" && r.faction === "M\xE9chant");
  ...
  const acolytePool = eligible.filter(
    (r) => r.faction === "M\xE9chant" && r.type !== "TUEUR" && !slugs.includes(r.slug)
  );
...
async function isGenericMechantKiller(slug) {
  ...
  const r = data;
  return !!r && r.faction === "M\xE9chant" && r.type === "TUEUR";
}
async function promoteAcolyteToTueur(gameId, temporary) {
  ...
  const realAcolytes = acolytes.filter((a) => {
    ...
    return info && info.type !== "TUEUR" && info.faction === "M\xE9chant";
  });
  ...
  const dispatchResult = await (async () => {
    switch (slug) {
      ...
      case "assistant_du_detective": {
        ...
        const isPickable = (r) => !EXCLUDED_SLUGS.has(r.slug) && !r.is_special && !r.emergent && r.type !== "BOULET";
        const targetFT = {
      ...
      case "apothicaire": {
        ...
        if (t1.id === actor.id)
          return {
            ok: false,
            message: "Offre la fiole \xE0 un autre joueur (pour l'utiliser toi-m\xEAme, passe par ton Carnet)."
          };
        ...
        if (available.length === 0)
          return { ok: false, message: "Toutes tes fioles ont d\xE9j\xE0 \xE9t\xE9 utilis\xE9es." };
        ...
        const apoMeta = apoMetaRow.data?.role_meta ?? {};
        const apoInv = (apoMeta.inventory ?? []).slice();
        ...
        await supabase.from("players").update({
          role_meta: {
            ...
            fioles_given: (apoMeta.fioles_given ?? 0) + 1,
